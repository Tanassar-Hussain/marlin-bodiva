...
export class ExchangeMarketSecurityPage implements OnInit {
  ...
  constructor(private appState: AppState, private loader: FuseLoaderScreenService, private listingService: ListingService, private _fb: FormBuilder, private translate: TranslateService, public userService: AuthService2) {
    this.clearFields();
  ...
  public onEditAction() {
    ...
    if (!AppUtility.isEmpty(this.exchangeMarketSecurityDetailList)) {
      ...
      this.selectedAssetId = this.selectedExchangeMarketSecurity.assetId.valueOf();
      this.selectedExchangeMarketSecurity.assetId = this.selectedAssetId;
  ...
  public onGeneratePaymentSchedualAction() {
    ...
    this.loader.show();
    this.listingService.getPaymentScheduleForSecurity(this.selectedExchangeMarketSecurity.securityId,true)
      .subscribe(
        restData => {
  ...
  onAssetChangeEvent(assetId): void {
    
    if (!AppUtility.isEmpty(assetId)) {
      this.getSecurityByAssetClass(this.selectedExchangeMarketSecurity.exchangeId, assetId)
    }
    else {
      this.securityList = null;
    }
          ...
          restData => {
            ...
            if (se.securityType.securityTypeId == AppConstants.SECURITY_TYPE_BOND) {
              ...
              if (!AppUtility.isEmpty(this.selectedExchangeMarketSecurity.exchangeId) && !AppUtility.isEmpty(this.selectedExchangeMarketSecurity.marketId)) {
                this.loader.show();
                this.listingService.getPaymentScheduleForSecurity(selectedId,true)
                  .subscribe(
                    restData => {
                      this.loader.hide();
                      this.bondPaymentSchedualList = restData;
      ...
      if (this.isEditing) {
        this.listingService.updateExchangeMarketSecurity(this.selectedExchangeMarketSecurity).subscribe(
          ...
          err => {
            this.loader.hide();
            if (err.message) {
              this.dialogCmp.statusMsg = <any>err.message;
            }
            else {
              this.dialogCmp.statusMsg = <any>err;
            }
  ...
  getAssetClassList() {
    ...
    let asset = {
      ...
      assetCode: AppConstants.PLEASE_SELECT_STR
    }
    ...
    this.listingService.getAssetClass().subscribe((res) => {
      ...
      if (this.isEditing) {
        ...
        else if (AppUtility.isValidVariable(res)) {
          this.assetClassList = res
          this.assetClassList.unshift(asset)
        }
      }
  ...
  getSecurityByAssetClass(exchangeId: number, assetId: number) {
    
    this.loader.show();
    if (!AppUtility.isEmpty(assetId)) {
      this.listingService.getSecuritiesByAssetClass(exchangeId, assetId).subscribe(res => {
        ...
        if (this.isEditing) {
          ...
          let isIncluded = this.securityList.some(res=>res.securityId === this.securityId);
          setTimeout(() => {
            if(isIncluded){
              this.selectedExchangeMarketSecurity.securityId = this.securityId;
              this.loader.hide();
              ...
              this.securityList.unshift(sec);
              this.selectedExchangeMarketSecurity.securityId = this.securityList[0].securityId
              this.loader.hide();
            }
          }, 150);
