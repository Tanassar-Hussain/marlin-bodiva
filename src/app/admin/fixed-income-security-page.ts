...
export class FISecurityPage implements OnInit {
  ...
  disableZeroCoupon: boolean = false;
  ...
  public onEditAction() {
    ...
    if (!AppUtility.isEmpty(this.selectedItem)) {
      ...
      if (!AppUtility.isValidVariable(this.selectedItem.securityFisDetail.indexedCurrency)) {
        this.selectedItem.securityFisDetail.indexedCurrency = new Currency();
      }

      if (!AppUtility.isValidVariable(this.selectedItem.securityFisDetail.couponFrequency)) {
        this.selectedItem.securityFisDetail.couponFrequency = new CouponFrequency();
      }
      
      if(!AppUtility.isValidVariable(this.selectedItem.securityFisDetail.bondSubCategory)){
  ...
  public onSaveAction(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      if (AppUtility.isValidVariable(this.selectedItem.securityFisDetail.indexedCurrency)) {
        if (this.selectedItem.currency.currencyId === this.selectedItem.securityFisDetail.indexedCurrency.currencyId) {
          if (this.lang === 'pt') {
            this.dialogCmp.statusMsg = 'A moeda e a moeda indexada devem ser diferentes.';
          }
          else {
      ...
      let exchange = new SecurityExchange();
      ...
      this.selectedItem.exchange = exchange;
