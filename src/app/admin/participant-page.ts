...
import { SubscriptionParticipant } from 'app/models/subscription_participant';
...
export class ParticipantPage implements OnInit {
  ...
  private validSubscription : boolean = false;
  ...
  @ViewChild('subscriptionTab', { static: false }) subscriptionTab : ElementRef;
...
public onChangeSubscription=(event)=>{
     if(AppUtility.isValidVariable(event) && AppUtility.isValidVariable(this.selectedItem.participantSubscriptions.pricing.billingId)){
           this.getSubscriptionRecord(event , this.selectedItem.participantSubscriptions.pricing.billingId);
     }
}


public onChangeBilling=(event)=>{
  if(AppUtility.isValidVariable(event) && AppUtility.isValidVariable(this.selectedItem.participantSubscriptions.pricing.subscriptionId)){
    this.getSubscriptionRecord(this.selectedItem.participantSubscriptions.pricing.subscriptionId, event);
}
}
...
public getSubscriptionRecord = (subscriptionId , billingId) => {
  this.subScriptionBillingData = [];
  this.selectedItem.participantSubscriptions.pricing.price = 0;
  this.selectedItem.participantSubscriptions.pricing.currencyId = null;
   this.listingService.getSubscriptionRecordByID(subscriptionId , billingId).subscribe((restData : any)=>{
               if(AppUtility.isValidVariable(restData) && !AppUtility.isEmptyArray(restData)){
                this.subScriptionBillingData = restData;
               }
  }, error => {
         if(error.message){
...
public onChangeCurrency=(event)=>{
  this.selectedItem.participantSubscriptions.pricing.price = 0;
   if(AppUtility.isValidVariable(event)){
      if(!AppUtility.isEmptyArray(this.subScriptionBillingData)){
                this.subScriptionBillingData.forEach((element)=>{
                   if(element.currencyId === event){
                      this.selectedItem.participantSubscriptions.pricing.price = element.price;
                      this.selectedItem.participantSubscriptions.netAmount = element.price;
                      this.selectedItem.participantSubscriptions.pricing.subscriptionPricingId = element.subscriptionPricingId;
                   }
                })
      }
   }
...
public changeDiscount = (event) => {
   if(AppUtility.isValidVariable(event) && AppUtility.isValidVariable(this.selectedItem.participantSubscriptions.pricing.price)){
         let price_ = 0;
         let discount = 0;
         discount = event / 100;
         price_  =  (this.selectedItem.participantSubscriptions.pricing.price * discount) ;
         this.selectedItem.participantSubscriptions.netAmount = this.selectedItem.participantSubscriptions.pricing.price - price_;
    }
    else{
      this.selectedItem.participantSubscriptions.netAmount = this.selectedItem.participantSubscriptions.pricing.price;
    }
}
  ...
  public clearFields() {
    ...
    this.selectedItem.clientOrderApproval = false;
 
    this.selectedItem.participantSubscriptions = new SubscriptionParticipant();
    this.selectedItem.participantSubscriptions.startDate =  new Date();
    this.selectedItem.participantSubscriptions.discount = null;
    this.selectedItem.participantSubscriptions.netAmount  = null;
    this.selectedItem.participantSubscriptions.active = true;
    this.selectedItem.participantSubscriptions.intimationDays = 0;
    this.selectedItem.participantSubscriptions.showWarning = true;

    this.selectedItem.participantSubscriptions.pricing = new SubscriptionPricing();
    this.selectedItem.participantSubscriptions.pricing.subscriptionId = null;
    this.selectedItem.participantSubscriptions.pricing.price = null;
    this.selectedItem.participantSubscriptions.pricing.userId = AppConstants.userId;
    this.selectedItem.participantSubscriptions.pricing.userName = AppConstants.username;
    ...
    this.selectedItem.user.defaultUser = true; // Recommended by Sir Hassan, Done by Usama Attique 
  ...
  public onPreviousAction() {
    if (this.currentTab_ == "CON") {
      ...
      this.subscriptionTab.nativeElement.click();
      this.currentTab_ = "SUB";
    }
    else if (this.currentTab_ == "SUB") {
  ...
  public onEditAction() {
    ...
    if (!AppUtility.isEmpty(this.itemsList.currentItem)) {
      ...
      this.getSubscriptionByParticpant(this.itemsList.currentItem.participantId);
    }
  }



  public getSubscriptionByParticpant=(particpantId : Number)=>{
     
     this.listingService.getSubscriptionByParticipant(particpantId).subscribe((res:any)=>{
      if(AppUtility.isValidVariable(res)){
        this.selectedItem.participantSubscriptions.participantId = res.participantId;
        this.selectedItem.participantSubscriptions.userId = res.userId;
        this.selectedItem.participantSubscriptions.participantSubscriptionId = res.participantSubscriptionId;
        this.selectedItem.participantSubscriptions.pricing.subscriptionId = res.pricing.subscriptionId;
        this.selectedItem.participantSubscriptions.pricing.billingId = res.pricing.billingId;
        this.selectedItem.participantSubscriptions.active = res.active;
        this.selectedItem.participantSubscriptions.startDate = res.startDate;
        this.selectedItem.participantSubscriptions.intimationDays = res.intimationDays;
        this.selectedItem.participantSubscriptions.showWarning = res.showWarning;
        setTimeout(() => {
          this.selectedItem.participantSubscriptions.pricing.currencyId =  res.pricing.currencyId;
        this.selectedItem.participantSubscriptions.pricing.price =  res.pricing.price;
        this.selectedItem.participantSubscriptions.discount =  res.discount;
        this.selectedItem.participantSubscriptions.netAmount =  res.netAmount;
        }, 250);
       }
  ...
  fromJSON(data: any) {
    //-------------------------------------------------------------
    if(AppUtility.isValidVariable(data.depoUser)){
    ...
    this.selectedItem.bankCode = data.bankCode;
    ...
    this.selectedItem.clientOrderApproval = data.clientOrderApproval;
  ...
  private participantToJSON() {
    ...
    if(this.selectedItem.accountType === 'I'){
      if (!this.isEditing) {
        if(AppUtility.isValidVariable(this.selectedItem.contactDetail.middleName) && this.selectedItem.contactDetail.middleName !== ''){
          temp = {
            'participant': {
              ...
              'clientOrderApproval' : this.selectedItem.clientOrderApproval,
              'bankCode': this.selectedItem.bankCode,
              'participantName': this.selectedItem.contactDetail.firstName + " " + this.selectedItem.contactDetail.middleName + " " + this.selectedItem.contactDetail.lastName,
            ...
            //------------------------------------
            'participantSubscriptions' : this.selectedItem.participantSubscriptions,
          }
        }
        else{
          temp = {
            'participant': {
              ...
              'bankCode': this.selectedItem.bankCode,
              'clientOrderApproval': this.selectedItem.clientOrderApproval,
              'participantName': this.selectedItem.contactDetail.firstName + " " + this.selectedItem.contactDetail.lastName,
              'accountType': this.selectedItem.accountType,
              'contactDetail': {
                'address1': this.selectedItem.contactDetail.address1,
                'cellNo': this.selectedItem.contactDetail.cellNo,
      ...
      if (!this.isEditing) {
        temp = {
          'participant': {
            ...
            'bankCode': this.selectedItem.bankCode,
             'clientOrderApproval' : this.selectedItem.clientOrderApproval,
            'participantName': this.selectedItem.contactDetail.companyName,
            'accountType': this.selectedItem.accountType,
            'contactDetail': {
              'address1': this.selectedItem.contactDetail.address1,
              'cellNo': this.selectedItem.contactDetail.cellNo,
  ...
  private setSelectedTabForUpdate() {
    ...
    this.updatingSubscription = this.currentTab_ === 'SUB' ? true : false;
  ...
  public onUpdateSubscription() {
    this.compulsoryError = false;
    this.alertMessage = '';
    this.validSubscription = this.validateSubscription();
  ...
  public onSaveAction(model: any, isValid: boolean) {
    ...
    if (this.isEditing) {
      ...
      else if(this.updatingSubscription){
        if (this.validSubscription) {
          this.loader.show();
          this.selectedItem.participantSubscriptions.participantId = this.selectedItemParticipantId;
          this.selectedItem.participantSubscriptions.userId = AppConstants.userId;
          this.listingService.updateParticipantSubscription(this.selectedItem.participantSubscriptions).subscribe(
            data => {
              this.loader.hide();
              // this.clearFields();           // Defect id: 606 @ 03/APRIL/2017 - Sahib Yar
              this.dialogCmp.statusMsg = AppConstants.MSG_RECORD_UPDATED;
  ...
  public validateBasic() {
    ...
    if (!(<any>this.myForm).controls.bankCode.valid) errorArray.push('Bank Code');
    if (this.selectedItem.accountType != 'I') {
  ...
  private markDirtBasic() {
    ...
    (<any>this.myForm).controls.bankCode.markAsDirty();
  ...
  public validateSubscription() {
    this.markDirtySubscription();
    let errorArray: string[] = [];
    if (!(<any>this.myForm).controls.subscription.valid) errorArray.push('Subscription');
    if (!(<any>this.myForm).controls.billing.valid) errorArray.push('Billing');
    if (!(<any>this.myForm).controls.currency.valid) errorArray.push('Currency');
    if (!(<any>this.myForm).controls.subStartDate.valid) errorArray.push('Start Date');
    if (!(<any>this.myForm).controls.price.valid) errorArray.push('Price');
    if (!(<any>this.myForm).controls.intimationDays.valid) errorArray.push('intimationDays');
    if((<any>this.myForm).controls.intimationDays.value === 0) errorArray.push('intimationDays');
    if (errorArray.length > 0) {
      if (!this.isEditing && this.finalsave) {
        this.alertMessage = this.alertMessage.concat('Please enter valid ' + errorArray.join(", ") + ' on \'<b>Subscription Tab</b>\'<br/>');
        this.compulsoryError = true;
      }
      return false;
    } else return true;
  ...
  private markDirtySubscription() {
    (<any>this.myForm).controls.subscription.markAsDirty();
    (<any>this.myForm).controls.billing.markAsDirty();
    (<any>this.myForm).controls.currency.markAsDirty();
    (<any>this.myForm).controls.subStartDate.markAsDirty();
    (<any>this.myForm).controls.price.markAsDirty();    
    (<any>this.myForm).controls.intimationDays.markAsDirty();
  }
  ...
  public onExchangesCheckedEvent(exchageIds: any) {
    ...
    if(AppUtility.isValidVariable(exchageIds)){
  ...
  public onNextAction() {
    ...
    if (this.currentTab_ == 'BI') {
      ...
      if (this.validateExchangeAssociation()) this.subscriptionTab.nativeElement.click();
    } 
    else if (this.currentTab_ == 'M') {
      if (this.validateModules()) this.systemAccessTab.nativeElement.click();
    }
    else if (this.currentTab_ == 'SUB') {
      if( this.validateSubscription()) this.modulesTab.nativeElement.click();
    }
    else if (this.currentTab_ == 'SA') {
    ...
    this.listingService.getSubsriptionsType().subscribe((res:any)=>{
        if(!AppUtility.isEmptyArray(res)){
            this.subscriptionTypesList = res;
              ...
              this.selectedItem.participantSubscriptions.pricing.subscriptionId = this.subscriptionTypesList[0].subscriptionId;
        }
        else{
            this.subscriptionTypesList = [];
        }
    ...
    this.listingService.getSubscriptionsBilling().subscribe((res:any)=>{
        if(AppUtility.isValidVariable(res)){
            this.subscriptionBillingList = res;
              ...
              this.selectedItem.participantSubscriptions.pricing.billingId = this.subscriptionBillingList[0].billingId;
        }
        else{
            this.subscriptionBillingList = [];
        }
    ...
    this.listingService.getCurrencyList()
      .subscribe(
        restData => {
         ...
         this.selectedItem.participantSubscriptions.pricing.currencyId = this.currencyList[0].currencyId;
          ...
          if (AppUtility.isEmptyArray(restData)) {
            ...
            if (!AppUtility.isEmptyArray(restData)) {
              ...
              for (let i: number = 0; i < tempArray.length; i++) {
                ...
                tempModules.push(this.moduleList[index]);
                if (AppUtility.isValidVariable(this.modules) && !this.modules.containsFocus()){
                  this.modules.checkedItems = tempModules;
          ...
          if (!AppUtility.isEmptyArray(restData)) {
            ...
            for (let i: number = 0; i < tempArray.length; i++) {
              ...
              this.exchangeList[index].$checked = true; 
               this.exchangeList[index].selected = true;
               tempExchange.push(this.exchangeList[index]);
            }

            if (AppUtility.isValidVariable(this.exchanges) && !this.exchanges.containsFocus()){
              this.exchanges.checkedItems = tempExchange;
  ...
  private addFromValidations() {
    this.myForm = this._fb.group({
      ...
      passwordDU: ['', Validators.compose([Validators.required,Validators.minLength(4)])],
      ...
      subStartDate : ['', Validators.compose([Validators.required])],
      price : ['' , Validators.compose([Validators.required])],
      discount : [''],
      netPrice : [''],
      intimationDays : ['' , Validators.compose([Validators.required])],
      showWarning : [''],

      // usernameDU: [''],
      // passwordDU: ['', Validators.compose([PasswordValidators.repeatCharacterRegexRule(4),
      // PasswordValidators.alphabeticalCharacterRule(1),
      // PasswordValidators.digitCharacterRule(1),
      // PasswordValidators.lowercaseCharacterRule(1),
      // PasswordValidators.uppercaseCharacterRule(1)])],
