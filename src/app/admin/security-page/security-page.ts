...
export class SecurityPage implements OnInit {
  ...
  public showBondDetail: boolean = false;
  ...
  constructor(private appState: AppState, private listingService: ListingService, private _fb: FormBuilder, public userService: AuthService2, private cd: ChangeDetectorRef, private loader: FuseLoaderScreenService, private translate: TranslateService) {
    ...
    this.selectedItem.currency = new Currency();
  ...
  public clearFields() {
    ...
    if (!AppUtility.isEmptyArray(this.currencyList)) {
      this.selectedItem.currency.currencyId = this.currencyList[0].currencyId;
    }
  ...
  onSecurityTypeChangeEvent(slectedSecurityTypeId): void {
    ...
    if(AppUtility.isValidVariable(slectedSecurityTypeId)){
  ...
  public onSaveAction(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      if (this.selectedItem.securityType.securityTypeId == AppConstants.SECURITY_TYPE_BOND) {
        ...
        if (date1.getTime() > date2.getTime()) {
          if(this.lang == 'pt'){
            this.dialogCmp.statusMsg = 'A Data de Vencimento deve ser posterior à Data de Emissão.';
          }
          else{
        ...
        if (AppUtility.isEmpty(this.numberOfCoupons) || this.numberOfCoupons == 0) {
          if (this.selectedItem.securityFisDetail.couponFrequency.frequencyCode == "A") {
            if(this.lang == 'pt'){
              this.dialogCmp.statusMsg = 'A diferença entre as datas de emissão e vencimento deve ser de pelo menos 1 ano no caso de Frequência de Cupom \'ANUAL\'.';
            }
            else{
            ...
            if(this.lang == 'pt'){
              this.dialogCmp.statusMsg = 'A diferença entre as datas de emissão e vencimento deve ser de pelo menos 6 meses no caso de Frequência de Cupom \'BI-ANUAL\'.';
            }
            else{
            ...
            if(this.lang == 'pt'){
              this.dialogCmp.statusMsg = 'A diferença entre as datas de emissão e vencimento deve ser de pelo menos 3 meses no caso de Frequência de Cupom \'TRIMESTRE\'.';
            }
            else{
            ...
            if(this.lang == 'pt'){
              this.dialogCmp.statusMsg = 'A frequência do cupom é necessária.';
            }
            else{
  ...
  private populateSecurityDetailList() {
    this.loader.show();    
    this.listingService.getSecuritiesBySecurityType(AppConstants.SECURITY_TYPE_EQUITY)
      .subscribe(
        restData => {
          this.loader.hide();
