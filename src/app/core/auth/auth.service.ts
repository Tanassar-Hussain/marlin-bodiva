...
export class AuthService {
    ...
    initializeAppConstants() {
        ...
        AppConstants.PARTICPANT_BANK_CODE = this.claims?.participant.bankCode;
        ...
        AppConstants.USER_COUNTRY_ID = this.claims?.user.countryId;
        AppConstants.USER_COUNTRY_CODE = this.claims?.user.dialingCode
        AppConstants.USER_MOBILE = this.claims?.user.mobile;
        AppConstants.USER_EXCHANGE_ID = this.claims?.user.exchangeId;

        let splitName = this.claims?.user.loginName;
        const myArray = splitName.split(" ");
        AppConstants.INV_USERNAME = this.claims?.sub;

        if(!AppUtility.isEmptyArray(myArray)){
            if(myArray.length === 2){                    
                AppConstants.INV_FIRST_NAME = myArray[0];
                AppConstants.INV_LAST_NAME = myArray[1];
            }
            else if(myArray.length === 3){
                AppConstants.INV_FIRST_NAME = myArray[0] + " " + myArray[1];
                AppConstants.INV_LAST_NAME = myArray[2];
            }
            else if(myArray.length === 4){
                AppConstants.INV_FIRST_NAME = myArray[0] + " " + myArray[1];
                AppConstants.INV_LAST_NAME = myArray[2] + " " + myArray[3]
            }
            else{
                AppConstants.INV_FIRST_NAME = myArray[0];
                AppConstants.INV_LAST_NAME = myArray[1];
        ...
        let tT = sessionStorage.getItem('tradeType');
    ...
    processToken(res): boolean {
        ...
        if (res.token) {
            ...
            AppConstants.loginCount = res.loginCount;
    ...
    updateProfile(params) {
        let str = "vtrade/update-profile"
        return this._restService.post(false, str, params);
    }

    updatePassword(params) {
        let str = "vtrade/update-password"
        return this._restService.post(false, str, params);
    }
    ...
    trendingSymbolAllAssetClassWise(exchangeCode, assetClassId: number) {
        let str = "security/recent/all-securities/symbols/exch/" + exchangeCode + "/asset/" + assetClassId
        return this._restService.get(false, str);
    }

    trendingSymbolAllMarketWiseCount(exchangeCode, marketCode: string) {
        let str = "/exchange-market-securities/securityCount/" + exchangeCode + "/" + marketCode
        return this._restService.get(false, str);
    }

    trendingSymbolAllAssetClassWiseCount(exchangeCode, assetCode: Number) {
        let str = `/security/recent/all-securities-count/symbols/exch/${exchangeCode}/asset/${assetCode}`
        return this._restService.get(false, str);
    }
