...
export class Order {
    ...
    settlementValueCurrency: number = 0;
    repurchasePriceCurrency: number = 0;
    tempSettlementAmount : number = 0;
    ...
    sender_username : string = AppConstants.username;
  ...
  ///////////////////////////Repo Fields//////////////////////
    ...
    clearOrder(): void {
        ...
        this.settlementValueCurrency = 0;
        this.account = null;
        this.custodian = '';
        this.type = '';
        this.type_ = '';
        ...
        this.sender_username = AppConstants.username;
    ...
    setOrder(obj): void {
        ...
        this.actual_settlement_amount = (AppUtility.isEmpty(obj.settlement_amount) ? 0 : Number(obj.settlement_amount));  
        ...
        this.state_time = new Date(obj.state_time+'Z');
    ...
    public formatOrderSubmitMsg(marketType: string): string {
        ...
        if (marketType == 'Bond') {
            ...
            let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');

            if (this.lang == "en") {
                 str = 'Repo Order Submission of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
            }
            else if (this.lang == 'pt') {
                str = 'Envio de pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
            }
        }
        else {
            if (this.lang == "en") {
    ...
    formatOrderConfirmationMsg(data, marketType: string): AlertMessage {
        ...
        let in_ = " dentro "
        let of_ = " do "
        let RepoOrderInitiated = "Contrato de recompra iniciado: "
        ...
        if (!('symbol' in data)) {
            ...
            if (marketType == 'Bond') {
                ...
                price = (data.price > 0) ? wijmo.Globalize.formatNumber(Number(data.price), 'n4') : 'Market';
                volume = wijmo.Globalize.formatNumber(Number(data.volume), 'n0');
                if (data.state === 'submitted' || data.state === 'received') {
                    orderConfirmMsg = '';
                  
                    if(data.negotiated_order_state === 'Initiated'){
                        if (this.lang == "en") {
                            orderConfirmMsg = 'Repo Contract Initiated: ' + AppUtility.ucFirstLetter(data.side) + ' ' +
                                data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' yield with Price ' + price + ', Order No. ' + data.order_no + ' in ' +
                                data.market + ' of ' + data.exchange;
                        }
                        else if (this.lang == 'pt') {
                            orderConfirmMsg = RepoOrderInitiated + AppUtility.ucFirstLetter(side) + ' ' +
                                data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' rendimento com Preço ' + price + ', Número do Pedido. ' + data.order_no + in_ +
                                data.market + of_ + data.exchange;
                        }
                        alertMessage.type = 'success';
                     }   
                     else if (data.negotiated_order_state === 'Rejected') {
                        orderConfirmMsg = '';
                        alertMessage.type = 'danger';
                        if (this.lang == "en") {
                            orderConfirmMsg = 'Repo Contract Rejected: ' + AppUtility.ucFirstLetter(data.side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' yield with Price ' +
                                price + ' in ' + data.market + ' of ' + data.exchange;
                        }
                        else if (this.lang == 'pt') {
                            orderConfirmMsg = 'Contrato de recompra rejeitado' + AppUtility.ucFirstLetter(side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' rendimento com Preço ' +
                                price + in_ + data.market + of_ + data.exchange;
                        }
                        alertMessage.type = 'danger';
                    }
                    else if (data.negotiated_order_state === 'Accepted') {
                        orderConfirmMsg = '';
                        alertMessage.type = 'danger';
                        if (this.lang == "en") {
                            orderConfirmMsg = 'Repo Contract Accepted: ' + AppUtility.ucFirstLetter(data.side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' yield with Price ' +
                                price + ' in ' + data.market + ' of ' + data.exchange + ', ' + data.rejection_message;
                        }
                        else if (this.lang == 'pt') {
                            orderConfirmMsg = 'Contrato de recompra aceito' + AppUtility.ucFirstLetter(side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' rendimento com Preço ' +
                                price + in_ + data.market + of_ + data.exchange + ', ' + rejectionMsg;
                        }
                        alertMessage.type = 'success';
                    }
                    else if (data.negotiated_order_state === 'PartiallyAccepted') {
                        orderConfirmMsg = '';
                        alertMessage.type = 'danger';
                        if (this.lang == "en") {
                            orderConfirmMsg = 'Repo Contract Partially Accepted: ' + AppUtility.ucFirstLetter(data.side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' yield with Price ' +
                                price + ' in ' + data.market + ' of ' + data.exchange + ', ' + data.rejection_message;
                        }
                        else if (this.lang == 'pt') {
                            orderConfirmMsg = 'Contrato de recompra parcialmente aceito' + AppUtility.ucFirstLetter(side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' rendimento com Preço ' +
                                price + in_ + data.market + of_ + data.exchange + ', ' + rejectionMsg;
                        }
                        alertMessage.type = 'success';
                    }
                    ...
                    alertMessage.type = 'danger';
                    if (this.lang == "en") {
                        orderConfirmMsg = 'Repo Contract Cancelled: ' + AppUtility.ucFirstLetter(data.side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' yield with Price ' +
                            price + ', Order No. ' + data.order_no + ' in ' + data.market + ' of ' + data.exchange;
                    }
                    else if (this.lang == 'pt') {
                        orderConfirmMsg = 'Contrato de recompra cancelado' + AppUtility.ucFirstLetter(side) + ' ' + data.symbol + ' ' + volume + ' @ ' + wijmo.Globalize.formatNumber(Number(data.yield), 'n4') + ' rendimento com Preço ' +
                            price + ', Número do Pedido. ' + data.order_no + in_ + data.market + of_ + data.exchange;
                    }
                    alertMessage.type = 'danger';
                }
    ...
    public formatREPOAffirmRejectCancelMsg = (marketType : any , state : string) => {
        let str: string = '';
        let price: string = '';
        let of_: string = " do ";
        let in_: string = " dentro ";
        ...
        if(marketType == 'Repo'){
            ...
            if(state === 'A'){
                let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');
                if (this.lang == "en") {
                     str = 'Repo Order Affirm of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
                }
                else if (this.lang == 'pt') {
                    str = 'Confirmação de pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
                }
            }
            else if(state === 'R'){
                let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');
                if (this.lang == "en") {
                     str = 'Repo Order Reject of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
                }
                else if (this.lang == 'pt') {
                    str = 'Rejeição de pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
                }
            }
            else if(state === 'C'){
                let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');
                if (this.lang == "en") {
                     str = 'Repo Order Cancel of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
                }
                else if (this.lang == 'pt') {
                    str = 'Cancelamento de pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
                }
            }
            else if(state === 'E'){
                let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');
                if (this.lang == "en") {
                     str = 'Repo Order Extend of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
                }
                else if (this.lang == 'pt') {
                    str = 'Extensão do pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
                }
            }
            else if(state === 'RP'){
                let sP = wijmo.Globalize.formatNumber(Number(this.sellPrice), 'n4');
                if (this.lang == "en") {
                     str = 'Repo Order Reprice of ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Sell Price ' + sP + '?';
                }
                else if (this.lang == 'pt') {
                    str = 'Repreço do pedido de recompra de ' + this.symbol.toUpperCase() + ' @ ' +  price + ' , Preço de venda ' + sP + '?';
                }
            }
