 ...
 *********************************/
  ...
  public clearFields() {
    ...
    this.selectedItem.defaultUser = false
  ...
  public onEditAction() {
    ...
    if (!AppUtility.isEmpty(this.selectedItem)) {
      ...
      if (this.selectedItem.userTypeId == AppConstants.USER_TYPE_AGENT)
        ...
        this.getClientDetail(this.userTypeClientId);
      }
      ...
      if (AppUtility.isValidVariable(this.stMultiSelect) && !this.stMultiSelect.containsFocus())
        this.stMultiSelect.checkedItems = checkedItems;
      ...
      this.listingService.getDashboardsbyParticipantAndUserTypeAndRoles(AppConstants.participantId, this.selectUserTypeId, this.strTemp)
        .subscribe(
          ...
          if(!AppUtility.isValidVariable(this.selectedItem.dashboard)){
            this.selectedItem.dashboard = "BACKOFFICE";
          }
    }
  }
...
public getClientDetail = (clientId : Number) => {
  this.clientDisplayName = "";
  this.listingService.getClientById(AppConstants.participantId , clientId).subscribe((res : any)=>{
            if(AppUtility.isValidVariable(res)){
              this.clientDisplayName = res.displayValue_;
            }
            else{
              this.clientDisplayName = "";
            }
  ...
  public onUserTypeChangeEvent(selectedTypeId): void {
    ...
    if (!AppUtility.isEmpty(selectedTypeId)) {
      ...
      else if (selectedTypeId == AppConstants.USER_TYPE_PARTICIPANT)
        this.populateDashboardList();
  ...
  **************************************/
    ...
    this.listingService.getUserTypeList()
      .subscribe(
        restData => {
          ...
          this.userTypeList = this.userTypeList.filter(user => user.userTypeId !== 2); //PARTICIPANT
        },
  ...
  private populateDashboardList() {
    ...
    if (this.selectUserTypeId !== 3 && this.selectUserTypeId !== 2)  // it means this isn't Participant-Admin and Participant
      this.selectedRoles();
    else
      this.strTemp = null;
    this.listingService.getDashboardsbyParticipantAndUserTypeAndRoles(AppConstants.participantId, this.selectUserTypeId, this.strTemp)
  ...
  private populateClientList() {
    if (AppUtility.isEmpty(this.clientList)) {
      this.listingService.getClientListByBrokerWithoutUser(AppConstants.participantId)
        .subscribe(
          restData => {
            if (AppUtility.isEmptyArray(restData)) {
              this.errorMessage = AppConstants.MSG_NO_DATA_FOUND;
