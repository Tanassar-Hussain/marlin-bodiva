...
export class AccountHoldingSummary implements OnInit {
  ...
  reportType: string = AppConstants.REPORT_TYPE_FORMAT_1;
  reportTypeList: any[] = [];
  agentList: any[] = [];
  ...
  constructor(private reportService: ReportService, private userService: AuthService2, private listingService: ListingService, private _fb: FormBuilder, private _fb2: FormBuilder, private appState: AppState, private translate: TranslateService, private loader: FuseLoaderScreenService) {
    ...
    this.isSMS = AppConstants.CONFIGURE_SMS_FLAG;
 ...
 *********************************/
  ...
  public onExchangeChange(val) {
    ...
    this.loadmarketList(this.exchangeId);

  }

  public onAssetClassChange(assetId) {
    if (assetId >= 0) {
      this.getSecuritiesByAssetClass(assetId)
    }
    else {
      this.getSecuritiesByExchange(this.exchangeId);
    }

  }

  public onMarketChange(marketId) {
    this.getSecuritiesByExchangeMarket(this.exchangeId, marketId)
  }
  ...
  public getSecuritiesByAssetClass(assetId) {
    // Populate market combo
    this.securityList = [];
    if (AppUtility.isValidVariable(assetId)) {

      this.listingService.getSecurityListByAssetClass(assetId)
        .subscribe(
          restData => {
  ...
  public getSecuritiesByExchangeMarket(exchangeId, marketId) {
    // Populate market combo
    this.securityList = [];
    if (AppUtility.isValidVariable(marketId)) {

      this.listingService.getSecurityByExchangeMarket(exchangeId, marketId)
        .subscribe(
          restData => {
        ...
        if (this.reportType.toLowerCase() == 'format-1' || this.reportType.toLowerCase() == 'formato-1') {
          ...
          if(this.email){
            ...
            this.reportService.getAccountHolding(this.params.PARTICIPANT_ID, startDate, endDate, this.params.EXCHANGE_ID,
              exchange, this.params.ASSET_ID, this.params.MARKET_ID, this.params.PARTICIPANT_BRANCH_ID, branch, encodeURIComponent(this.params.FROM_ACCOUNT.valueOf()),
          ...
          if (this.email) {
            ...
            this.AccountHoldingFormatWithOutEmail(this.params, exchange, this.params.ASSET_ID, this.params.MARKET_ID, security, branch, startDate, endDate, agent, this.lang);
          }
        }
          ...
          if (this.email) {
            ...
            this.AccountHoldingFormat2WithOutEmail(this.params, exchange, this.params.ASSET_ID, this.params.MARKET_ID, security, branch, startDate, endDate, agent, this.lang);
          }
        }
      }
    }
  ...
  public AccountHoldingFormatWithOutEmail(params, exchange, assetId, marketId, security, branch, startDate, endDate, agent, lang) {
    this.reportService.getAccountHoldingFormat1(params.PARTICIPANT_ID, startDate, endDate, params.EXCHANGE_ID,
      exchange, assetId, marketId, params.PARTICIPANT_BRANCH_ID, branch, encodeURIComponent(params.FROM_ACCOUNT.valueOf()),
  ...
  public AccountHoldingFormat2WithOutEmail(params, exchange, assetId, marketId, security, branch, startDate, endDate, agent, lang) {
  ...
  // ..........................................................changes..............................................................
  loadmarketList(exchangeId) {
    // update marketList data

    this.appState.showLoader = true;
    this.listingService.getmarketList(exchangeId).subscribe((data) => {
      this.appState.showLoader = false;
      if (data != null) {

        this.marketList = data;
        var market = { marketId: AppConstants.ALL_VAL, marketCode: AppConstants.ALL_STR }
        this.marketList.unshift(market);
        this.params.MARKET_ID = this.marketList[0].marketId
  ...
  loadAssetClassList() {
    ...
    this.listingService.getAssetClassList().subscribe((data) => {
      ...
      if (data != null) {
        ...
        var asset = { assetId: AppConstants.ALL_VAL, assetName: AppConstants.ALL_STR }
        this.assetClassList.unshift(asset);
        this.params.ASSET_ID = this.assetClassList[0].assetId
      }
    },
      (error) => {
        this.appState.showLoader = false;
