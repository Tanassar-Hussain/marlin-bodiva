...
export class RedemptionPaymentRpt implements OnInit{
    ...
    @ViewChild('cmbClient') cmbClient: wjcInput.ComboBox;
    ...
    loadSymbolMarketExchangeList(){
        ...
        if(AppUtility.isValidVariable(AppConstants.participantId)){
            ...
            this.loader.show();
            this.listingService.getexchangeCodeSecurityExchanges().subscribe(resData => {
                this.loader.hide();
                if(!AppUtility.isEmptyArray(resData)){
                   this.updateSecurityList(resData);
                } else {
    ...
    getRedemptionPaymentDetailRefresh(){
    ...
    getRedemptionPaymentDetail(value,isValid){
       ...
       if(isValid){
           ...
           this.loader.show()
           this.listingService.getRedemptionPaymentListNew(couponPayload).subscribe((resData) => {
              this.loader.hide();
              if(!AppUtility.isEmptyArray(resData)){
                 this.filterRedemptionPaymentList(resData,value);
              } else{
           ...
           if(selectedValues.security === AppConstants.ALL_VAL && selectedValues.clientCode === AppConstants.ALL_VAL){
            data.forEach(element => {
                if(AppUtility.isValidVariable(element.accountCode) && element.accountCode !== ""){
                    this.redemptionPaymentList.push(element);
                }
           });
