...
export class ManualTransactionPage implements OnInit {
    ...
    public isRequiredBondFields : boolean = false;
    public securityParValue : number = 0;
    ...
    securityIdEdit: number;
    public isCustodian  = AppConstants.CUSTODIAN_MODEL;
 ...
 public onChangePrice=(event)=>{
       if(AppUtility.isValidVariable(event)){
           if(this.isEditing){
               setTimeout(() => {
                if(!this.isRequiredBondFields){
                    this.selectedItem.tradeValue =  event * this.selectedItem.volume;
                 }
                 else if(this.isRequiredBondFields && (this.bondType === AppConstants.BOND_TYPE_ZERO_COUPON_ID)){
                     this.selectedItem.tradeValue =  event * this.selectedItem.volume;
                 }
                 else{
                     this.selectedItem.tradeValue = (event * this.selectedItem.volume * this.securityParValue) / 100;
                 }
               }, 150);
           }
           else{
  ...
  public onChangeVolume=(event)=>{
    
    if(this.isEditing){
        setTimeout(() => {
            if(AppUtility.isValidVariable(event)){
                if(!this.isRequiredBondFields){
                   this.selectedItem.tradeValue =  this.selectedItem.price * event;
                }
                else if(this.isRequiredBondFields && (this.bondType === AppConstants.BOND_TYPE_ZERO_COUPON_ID)){
                   this.selectedItem.tradeValue = this.selectedItem.price * event;
               }
               else{
                   this.selectedItem.tradeValue = (this.selectedItem.price * event * this.securityParValue) / 100;
               }
           }
        }, 150);
    }
    ...
    public onMarketChangeEvent(event): void {
         
              if(!this.isEditing){
                this.isRequiredBondFields = false;
                this.myForm.get('accruedProfit').clearValidators();
                this.myForm.get('settlementAmount').clearValidators();
                this.selectedItem.settlementAmount = 0;
                this.selectedItem.accruedProfit = 0;
                this.selectedItem.tradeValue = 0;
        ...
        if (AppUtility.isValidVariable(event)) {
            this.populateTransactionTypeList(this.selectedItem.exchangeId, event);
            this.populateSecurityList(this.selectedItem.exchangeId, event, this.selectedItem.settlementTypeId, true);
                ...
                restData => {
                    ...
                    if (AppUtility.isEmptyArray(restData)) {
                        ...
                        if(this.isEditing){
                            ...
                            this.symbolList.forEach(element => {
                                if(this.securityIdEdit === element.securityId){
                                    if(element.securityType.securityTypeId === AppConstants.SECURITY_TYPE_BOND){
                                  
                                    this.isRequiredBondFields = true;
                                    const ctrl: FormControl = (<any>this.myForm).controls.settlementAmount;
                                    ctrl.setValidators(Validators.required);
                                    ctrl.updateValueAndValidity();
                                    if (!AppUtility.isValidVariable(this.selectedItem.settlementAmount) || this.selectedItem.settlementAmount == 0) {
                                        this.myForm.controls['settlementAmount'].setErrors({ 'required': true });
                                    }
                    
                                    const ctrl1: FormControl = (<any>this.myForm).controls.accruedProfit;
                                    ctrl1.setValidators(Validators.required);
                                    ctrl1.updateValueAndValidity();
                                    if (!AppUtility.isValidVariable(this.selectedItem.accruedProfit) || this.selectedItem.accruedProfit == 0) {
                                        this.myForm.controls['accruedProfit'].setErrors({ 'required': true });
                                    }           
                                }
                                else{
                                    this.isRequiredBondFields = false;
    ...
    public onSecurityChangeEvent(selectedId): void {
        ...
        if(!this.isEditing){
            this.symbolList.forEach(element => {
                if(selectedId === element.securityId){
                    if(element.securityType.securityTypeId === AppConstants.SECURITY_TYPE_BOND){
    ...
    public onSearchTransaction(selectedStatus, transactionDate): void {
        ...
        if (AppUtility.isEmpty(selectedStatus)) {
            if(this.lang === 'pt'){
                this.dialogCmp.statusMsg = 'Selecione o status da transação';
            }
            else{
    ...
    public onEditAction() {
        ...
        this.securityIdEdit = this.selectedItem.securityId;
    ...
    public onPostAction(selectedStatus) {
        if (this.itemsList.itemCount < 1) {
            ...
            if (selectedStatus === 'CANCELED') {
                if(this.lang === 'pt'){
                    this.dialogCmp.statusMsg = 'Tem certeza de que deseja Cancelar todos os registros?';
                }
                else{
                ...
                if(this.lang === 'pt'){
                    this.dialogCmp.statusMsg = 'Tem certeza de que deseja cancelar a postagem de todos os registros?';
                }
                else{
                ...
                if(this.lang === 'pt'){
                    this.dialogCmp.statusMsg = 'Tem certeza de que deseja postar todos os registros?';
                }
                else{
    ...
    public onSaveAction(model: any, isValid: boolean) {
        ...
        if ((this.cmbTransactionType.text === 'MTSD' || this.cmbTransactionType.text === 'MFSD' || this.cmbTransactionType.text === 'MTSN' || this.cmbTransactionType.text === 'MFSN') && this.selectedItem.fcDays < 1) {
            ...
            if(this.lang === 'pt'){
                this.dialogCmp.statusMsg = 'Data do próximo calendário de liquidação não definida. Entre em contato com o administrador.';
            }
            else{
        ...
        // else if ((this.cmbTransactionType.text != 'MTSD'  this.cmbTransactionType.text == 'MFSD' || this.cmbTransactionType.text == 'MTSN' || this.cmbTransactionType.text == 'MFSN')) {
            ...
            if (isValid) {
                ...
                if (!modVal.isZero()) {
                    if(this.lang === 'pt'){
                        this.dialogCmp.statusMsg = 'Preço inválido, o preço deve ser múltiplo de ' + this.tickSize;
                    }
                    else{
                ...
                if ((this.selectedItem.volume % this.boardLot) !== 0) {
                    if(this.lang === 'pt'){
                        this.dialogCmp.statusMsg = 'Quantidade inválida, a quantidade deve ser múltipla de ' + this.boardLot;
                    }
                    else{
    ...
    **************************************/
    ...
    private populateExchangeMarketSecurityDetail(exchangeId: number, marketId: number, securityId: number) {
        ...
        if (AppUtility.isValidVariable(exchangeId)) {
            this.listingService.getSymbolMarket(exchangeId, marketId, securityId)
                .subscribe(
                    (restData : any) => {
                        this.loader.hide();
                        ...
                        if (!AppUtility.isValidVariable(restData)) {
                            ...
                            if(AppUtility.isValidVariable(restData.fisDetail)){

                                this.securityParValue =  restData.fisDetail.parValue;
                                // if(restData.fisDetail.indexedBond === true){
                                //     this.securityParValue =  restData.fisDetail.currentNominalValue;
                                // }

                                // if(restData.fisDetail.indexedBond === false){
                                //     this.securityParValue =  restData.fisDetail.parValue;
                                // }
                                
                            }
                            if(AppUtility.isValidVariable(restData.fisDetail) && AppUtility.isValidVariable(restData.fisDetail.bondType)){
                                this.bondType = restData.fisDetail.bondType.bondTypeId;
                            }

                                this.boardLot = restData.boardLot;
                                this.tickSize = restData.tickSize;
                        }
                    },
                    error => {
