 ...
 *********************************/
  ...
  reportTypeListArr() { 
    this.reportTypeList
    let levelArray = [];
    this.translate.get(['Translation.Format-1', 'Translation.Format-2']).subscribe((res: any) => {
      
      levelArray.push(res['Translation.Format-1']);
      levelArray.push(res['Translation.Format-2']);
     // levelArray.push('Format-2A');

      let levelCmbList = [];
      let cmbItem: ComboItem

      for (let i = 0; i < levelArray.length; i++) {
        cmbItem = new ComboItem(levelArray[i], levelArray[i]);
        levelCmbList[i] = cmbItem;
      }
      this.reportTypeList = levelCmbList;
