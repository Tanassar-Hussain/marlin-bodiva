...
export class VoucherRpt implements OnInit {
    ...
    @ViewChild('vouchers', { static: false }) vouchers: wjcInput.MultiSelect;
    allVouchers: boolean = false
    ...
    printReport(model: any, isValid: boolean) {
        ...
        if (this.selectedItem.START_DATE > this.selectedItem.END_DATE) {
            ...
            this.dateComparison = true;
            if (this.vouchertypeid.checkedItems.length > 0 || this.allVouchers) {
                this.vouchertype = true;
                isValid = true;
                if (isValid) {
    ...
    private selectedVoucherTypes() {
        ...
        if (!this.allVouchers) {
            ...
            for (let i = 0; i < this.voucherTypeList.length; i++) {
                items.push(this.voucherTypeList[i].voucherTypeId);
                items_Name.push(this.voucherTypeList[i].typeDesc);
            }
            this.selectedItem.IDS = items;
            this.selectedItem.NAMES = items_Name;
        }
    ...
    private populateVoucherTypeList() {
        this.listingService.getVoucherTypeList(AppConstants.participantId)
            .subscribe(
                ...
                error => this.dialogCmp.statusMsg = <any>error.message);
    }
    ...
    public onAllSelected(e) {
        if (e.target.checked) {
            this.vouchers.isDisabled = true;
            this.allVouchers = true;
        } else {
            this.vouchers.isDisabled = false;
            this.allVouchers = false;
        }
    }
