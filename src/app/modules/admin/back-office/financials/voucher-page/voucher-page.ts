...
export class VoucherPage implements OnInit {
    ...
    constructor(private appState: AppState, private listingService: ListingService, private _fb: FormBuilder, 
        ...
        switch (this.lang) {
            case "en":
                this.voucherStatusList = [ { "name": "New", "code": "UNPOSTED" },  { "name": "Post", "code": "POSTED" },
                ]
                ...
                break;
            case "pt":
                this.voucherStatusList = [  { "name": "Nova", "code": "UNPOSTED" },  {  "name": "Publicar", "code": "POSTED" },
               ]
    ...
    ngOnInit() {
        ...
        this.addFormValidations();
            ...
            this.onSearchvoucher(null, 'UNPOSTED', new Date(), new Date());
   ...
   *********************************/
    ...
    private ClearMasterObject() {
        ...
        this.selectedItem.debitCredit = 0.0000;
    ...
    private ClearDetailObject() {
        ...
        this.selectedDetailItem.debitCredit = 0.0000;
    ...
    public onAddNewRow() {
        ...
        if (this.fiscalYearExist) {
            if (!this.isInvalidcode) {
                if (!this.isDetailEditing) {
                    if (AppUtility.isValidVariable(this.voucherDetailList) && this.voucherDetailList.items.length && this.myForm.valid) {
                        if (this.isUnique(false)) {
                            ...
                            this.dialogCmp.statusMsg = AppConstants.MSG_ACCOUNT_ALREADY_EXIST;
                ...
                this.dialogCmp.statusMsg = AppConstants.MSG_INVALID_CODE;
                this.dialogCmp.showAlartDialog('Warning');
            }
        }

        else {
            this.dialogCmp.statusMsg = AppConstants.MSG_VOU_DATE_BETWEEN_FIS;
            this.dialogCmp.showAlartDialog('Warning');
        }
    }
    ...
    public onEditDetailDelete() {
        if (this.voucherDetailList.items.length > 1) {
            ...
            this.dialogCmp.statusMsg = AppConstants.MSG_ONE_ITEM_REQUIRED_VOUCHER;
            this.dialogCmp.showAlartDialog('Warning');
        }
    }
    ...
    public onSaveAction(model: any, isValid: boolean) {
        ...
        if (!AppUtility.isEmpty(this.selectedItem.voucherMaster.vouNaration) && !AppUtility.isEmpty(this.selectedItem.voucherMaster.voucherType.voucherTypeId) && !AppUtility.isEmpty(this.selectedItem.voucherMaster.paymentType)) {
            if (this.flexDetail.columnFooters.getCellData(0, 2, true) == this.flexDetail.columnFooters.getCellData(0, 3, true)) {
                    ...
                    if (this.isEditing) {
                        ...
                        if (AppUtility.isValidVariable(this.voucherDetailList)) {
                            ...
                            //    let a = (element.voucherMaster.vouDate).toLocaleDateString();
                                element.voucherMaster.vouDateStr = this.datePipe.transform(element.voucherMaster.vouDate , 'yyyy-MM-dd');
                             })
                ...
                this.loader.hide();
                this.dialogCmp.statusMsg = AppConstants.MSG_DEBIT_CREDIT_EQUAL;
                this.dialogCmp.showAlartDialog('Warning');
            }
        }
        else {
            ...
            .subscribe(
                restData => {
                  ...
                  //  this.voucherTypeList = restData;
                     this.voucherTypeListSearch = restData;

                    restData.forEach(element => {
                         this.voucherTypeList.push(element);
                    });

                    let vt: VoucherType = new VoucherType();
                    vt.voucherTypeId = AppConstants.PLEASE_SELECT_VAL;
                    vt.typeDesc = AppConstants.PLEASE_SELECT_STR;
                    this.voucherTypeList.unshift(vt);
                
                    let vts: VoucherType = new VoucherType();
                    vts.voucherTypeId = AppConstants.PLEASE_SELECT_VAL;
                    vts.typeDesc = AppConstants.ALL_STR;
                    this.voucherTypeListSearch.unshift(vts);
                },
    ...
    public onSearchvoucher(voucherTypeId, voucherStatus, fromDate, ToDate): void {
        ...
        if (AppUtility.isEmpty(voucherStatus)) {
            if(this.lang === 'en'){
                ...
                this.dialogCmp.statusMsg = 'Selecione o status do comprovante.';
            }
            this.dialogCmp.showAlartDialog('Warning');
            ...
            this.dialogCmp.statusMsg = AppConstants.MSG_FROM_DATE_LESS_TO_DATE;
            this.dialogCmp.showAlartDialog('Warning');
    ...
    public onPostAction(selectedStatus) {
        ...
        if (this.checkedSelectedItem.length < 1) {
            ...
            for (var i = 0; i < this.checkedSelectedItem.length; i++) {
                if ((this.checkedSelectedItem[i].chequeCleared && this.checkedSelectedItem[i].voucherType.voucherType == 'BRV') || this.checkedSelectedItem[i].voucherType.voucherType != 'BRV') {
                    this._items.push(this.checkedSelectedItem[i].voucherMasterId);
                    
                }
                else
                    voucherMasterIdStr = this.checkedSelectedItem[i].vouNo + ", " + voucherMasterIdStr
            }
            ...
            if (this._selectedStatus == "REVERSED") {
                ...
                if (AppUtility.isEmptyArray(this._items)) {
                    this.dialogCmp.statusMsg = AppConstants.MSG_CHEQUE_NOT_CLEARED + " " +voucherMasterIdStr.slice(0, -2);
                    this.dialogCmp.showAlartDialog('Warning');
                }
                else {
                    if (!AppUtility.isEmpty(voucherMasterIdStr)) {
                        showMsg = AppConstants.MSG_CHEQUE_NOT_CLEARED + " " + voucherMasterIdStr.slice(0, -2);

                        showMsg = showMsg + "\n"+AppConstants.MSG_CONFIRM_POST_REMAINING_RECORD;
                    }
                    else
                        showMsg = showMsg + "\n"+AppConstants.MSG_CONFIRM_POST_ALL_RECORD ;
            ...
            .subscribe(
                restData => {
                    ...
                    if (AppUtility.isEmptyArray(restData)) {
                        ...
                        this.selectedDetailItem.chartOfAccount.glDesc = AppConstants.MSG_INVALID_CODE;
                        this.selectedDetailItem.chartOfAccount.glCode = null;
                        this.selectedDetailItem.chartOfAccount.chartOfAccountId = null;
                    } else {
        ...
        if (btnClicked == 'Success')
            ...
            if (this.isDeleting) {
                if (AppUtility.isValidVariable(this.itemsList.currentItem.voucherMasterId)) {
                    ...
                    this.loader.hide();
                    this.dialogCmp.statusMsg = AppConstants.MSG_NO_ACTION_HAS_TAKEN;
