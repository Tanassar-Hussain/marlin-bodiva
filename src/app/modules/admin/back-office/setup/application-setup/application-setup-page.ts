...
import { ParticipantDocuments } from 'app/models/participantDocuments';
...
export class ApplicationSetupPage implements OnInit, AfterViewInit {
    ...
    public fileSelectionMsgShowTCen = false;
    public fileSizeExceedTCen = false;
    public fileSelectionMsgShowTCpt = false;
    public fileSizeExceedTCpt = false;
    ...
    public cashVoucherDebitText: String = 'Cash Voucher Debit Text';
    public cashVoucherCreditText: String = 'Cash Voucher Credit Text';
    public ShowTermsAndConditionsText : String = 'Show Terms and Conditions for Participant';
    ...
    private fileContentTypeTC_en : String = '';
    private file_srcsTCen: string[] = [];
    public fileNameTC_en: String = '';

    private fileContentTypeTC_pt : String = '';
    private file_srcsTCpt: string[] = [];
    public fileNameTC_pt: String = '';

    public selectedTermsAndConditionsEn : ParticipantDocuments;
    public selectedTermsAndConditionsPt : ParticipantDocuments;
    ...
    public validDebitVoucherText : boolean = true;
    public validCreditVoucherText : boolean = true;
    ...
    private fileInputTC_en: any;
    private fileInputTC_pt: any;
    ...
    public checkString = (input: string): boolean => {
        return this.pattern.test(input);
      };


 
     public onChangeDebitVoucherText = () => {
        if(!AppUtility.isNullOrEmpty(this.selectedItem.cashVoucherDebitText)){
           this.validDebitVoucherText =  this.checkString(this.selectedItem.cashVoucherDebitText); // true     
        }
        
      }


      public onChangeCreditVoucherText = () => {
        if(!AppUtility.isNullOrEmpty(this.selectedItem.cashVoucherCreditText)){
            this.validCreditVoucherText =   this.checkString(this.selectedItem.cashVoucherCreditText); // true     
        }
      }
   ...
   *********************************/
    ...
    fileChangeEventTCen(input) {
     
    
        this.file_srcsTCen = [];
        if (AppUtility.isEmpty(input)) {
            this.fileSelectionMsgShowTCen = true;
            return;
        } else
            this.fileSelectionMsgShowTCen = false;

        if (!AppUtility.isEmpty(input.files[0])) {
            if (input.files[0].size > 26214400) {
                this.fileSizeExceedTCen = true;
                return;
            } else {
                this.fileSizeExceedTCen = false;
                this.fileContentTypeTC_en = input.files[0].type;
            }
        } else {
            this.file_srcsTCen = [];
            return;
        ...
        if (fullPath) {
            ...
            this.fileNameTC_en = filename;
        }
        var _validFileExtensions = ['.pdf'];
        if (this.fileNameTC_en.length > 0) {
            var blnValid = false;
            for (var j = 0; j < _validFileExtensions.length; j++) {
                var sCurExtension = _validFileExtensions[j];
                if (this.fileNameTC_en.substr(this.fileNameTC_en.length - sCurExtension.length, sCurExtension.length).toLowerCase() == sCurExtension.toLowerCase()) {
                    blnValid = true;
                    break;
                }
            }
            if (!blnValid) {
                ...
                this.fileNameTC_en = '';
                this.fileContentTypeTC_en = '';
                input.value = '';
                return;
            }
        }

        this.readFilesTCen(input.files);
        this.fileInputTC_en = input;
        this.selectedTermsAndConditionsEn.documentBase64_ = input; //this.file_srcs[0];
        this.selectedTermsAndConditionsEn.documentName  = this.fileNameTC_en;
        this.selectedTermsAndConditionsEn.contentType = this.fileContentTypeTC_en;
        this.selectedTermsAndConditionsEn.langId = AppConstants.LANGUAGE_ID_ENGLISH;
        this.selectedTermsAndConditionsEn.participantId = AppConstants.participantId;
    }


    readFileTCen(file, reader, callback) {
        // Set a callback funtion to fire after the file is fully loaded
        reader.onload = () => {
            // callback with the results
            callback(reader.result);
    ...
    readFilesTCen(files, index = 0) {
        // Create the file reader
        let reader = new FileReader();
        // If there is a file
        if (index in files) {
            // Start reading this file
            this.readFileTCen(files[index], reader, (result) => {
                // After the callback fires do:
                this.file_srcsTCen = [];
                this.selectedTermsAndConditionsEn.documentBase64_ = result;
                this.file_srcsTCen.push(result);
                this.readFilesTCen(files, index + 1);// Read the next file;
            });
            this.changeDetectorRef.detectChanges();
        } else {
            // When all files are done This forces a change detection
    ...
    fileChangeEventTCpt(input) {

        this.file_srcsTCpt = [];
        if (AppUtility.isEmpty(input)) {
            this.fileSelectionMsgShowTCpt = true;
            return;
        } else
            this.fileSelectionMsgShowTCpt = false;

        if (!AppUtility.isEmpty(input.files[0])) {
            if (input.files[0].size > 26214400) {    //26214400 is equal to 25MB
                this.fileSizeExceedTCpt = true;
                return;
            } else {
                this.fileSizeExceedTCpt = false;
                this.fileContentTypeTC_pt = input.files[0].type;
            }
        } else {
            this.file_srcsTCpt = [];
            return;
        ...
        if (fullPath) {
            ...
            this.fileNameTC_pt = filename;
        }
        var _validFileExtensions = ['.pdf'];
        if (this.fileNameTC_pt.length > 0) {
            var blnValid = false;
            for (var j = 0; j < _validFileExtensions.length; j++) {
                var sCurExtension = _validFileExtensions[j];
                if (this.fileNameTC_pt.substr(this.fileNameTC_pt.length - sCurExtension.length, sCurExtension.length).toLowerCase() == sCurExtension.toLowerCase()) {
                    blnValid = true;
                    break;
                }
            }
            if (!blnValid) {
                ...
                this.fileNameTC_pt = '';
                this.fileContentTypeTC_pt = '';
                input.value = '';
                return;
            }
        }

        this.readFilesTCpt(input.files);
        this.fileInputTC_pt = input;
        this.selectedTermsAndConditionsPt.documentBase64_ = input; //this.file_srcs[0];
        this.selectedTermsAndConditionsPt.documentName  = this.fileNameTC_pt;
        this.selectedTermsAndConditionsPt.contentType = this.fileContentTypeTC_pt;
        this.selectedTermsAndConditionsPt.langId = AppConstants.LANGUAGE_ID_PORTUGUESE;
        this.selectedTermsAndConditionsPt.participantId = AppConstants.participantId;
    }



    readFileTCpt(file, reader, callback) {
        // Set a callback funtion to fire after the file is fully loaded
        reader.onload = () => {
            // callback with the results
            callback(reader.result);
    ...
    readFilesTCpt(files, index = 0) {
        ...
        if (index in files) {
            // Start reading this file
            this.readFileTCpt(files[index], reader, (result) => {
                // After the callback fires do:
                this.file_srcsTCpt = [];
                this.selectedTermsAndConditionsPt.documentBase64_ = result;
                this.file_srcsTCpt.push(result);
                this.readFilesTCpt(files, index + 1);// Read the next file;
            });
            this.changeDetectorRef.detectChanges();
        } else {
            // When all files are done This forces a change detection
    ...
    public clearFields() {
        ...
        if (!AppUtility.isEmptyArray(this.languageList)) {
            this.language1TypeId = this.languageList[0].languageCode;
        }
        if (!AppUtility.isEmptyArray(this.languageList)) {
            this.language2TypeId = this.languageList[1].languageCode;
        }
        if (!AppUtility.isEmptyArray(this.languageList)) {
            this.language2FooterTypeId = this.languageList[1].languageCode;
        }
        this.editDetailEquity = false;
        this.deleteDetailEquity = false;
        ...
        this.selectedTermsAndConditionsEn = new ParticipantDocuments();
        this.selectedTermsAndConditionsPt = new ParticipantDocuments();
    ...
    public onChartOfAccountLevelChange(chartOfAccountLevel: number) {
        if (AppUtility.isValidVariable(chartOfAccountLevel)) {
            if (chartOfAccountLevel.valueOf() < 1 || chartOfAccountLevel.valueOf() > 10) {
    ...
    public markDirtyStationeryTabControls() {
        ...
        if (this.file_srcsTCen.length == 0 || this.fileSizeExceedTCen)
            this.fileSelectionMsgShowTCen = true;

        if (this.file_srcsTCpt.length == 0 || this.fileSizeExceedTCpt)
            this.fileSelectionMsgShowTCpt = true;
    }
    ...
    public getParticipantParams(): any {
        ...
        param1.languageCode = this.languageList[0].languageCode
        param1.columnKey = "PARAM_VALUE"
        param1.language.languageId = this.languageList[0].languageId
        param1.language.languageCode = this.languageList[0].languageCode
        param1.language.languageName = this.languageList[0].languageName
        param1.language.countryCode = this.languageList[0].countryCode
        ...
        param3.languageCode = this.languageList[0].languageCode
        param3.columnKey = "PARAM_VALUE"
        param3.language.languageId = this.languageList[0].languageId
        param3.language.languageCode = this.languageList[0].languageCode
        param3.language.languageName = this.languageList[0].languageName
        param3.language.countryCode = this.languageList[0].countryCode
        ...
        param5.languageCode = this.languageList[0].languageCode
        param5.columnKey = "PARAM_VALUE"
        param5.language.languageId = this.languageList[0].languageId
        param5.language.languageCode = this.languageList[0].languageCode
        param5.language.languageName = this.languageList[0].languageName
        param5.language.countryCode = this.languageList[0].countryCode
        ...
        param7.languageCode = this.languageList[0].languageCode
        param7.columnKey = "PARAM_VALUE"
        param7.language.languageId = this.languageList[0].languageId
        param7.language.languageCode = this.languageList[0].languageCode
        param7.language.languageName = this.languageList[0].languageName
        param7.language.countryCode = this.languageList[0].countryCode
        ...
        param9.languageCode = this.languageList[0].languageCode
        param9.columnKey = "PARAM_VALUE"
        param9.language.languageId = this.languageList[0].languageId
        param9.language.languageCode = this.languageList[0].languageCode
        param9.language.languageName = this.languageList[0].languageName
        param9.language.countryCode = this.languageList[0].countryCode
        ...
        participantParams.push(
            {
                'paramId': 24,
                'paramValue': this.selectedItem.cashVoucherDebitText,
                'paramName': this.cashVoucherDebitText,
                'participant': JSON.parse(JSON.stringify(this.selectedItem.participant))
            }
        )
        participantParams.push(
            {
                'paramId': 25,
                'paramValue': this.selectedItem.cashVoucherCreditText,
                'paramName': this.cashVoucherCreditText,
                'participant': JSON.parse(JSON.stringify(this.selectedItem.participant))
            }
        )
        ...
        participantParams.push(
            {
                'paramId': 26,
                'paramValue': this.selectedItem.showTermsAndConditions,
                'paramName': this.ShowTermsAndConditionsText,
                'participant': JSON.parse(JSON.stringify(this.selectedItem.participant))
            }
        )
    ...
    public toJson(): any {
        ...
        let temp: any = {
            ...
            "participantDocuments" : this.selectedItem.participantDocuments
        }
    ...
    fillApplicationSetupFromJson(as: ApplicationSetup, data: any) {
        if (!AppUtility.isEmpty(data)) {
            ...
            if(!AppUtility.isEmptyArray(data.participantDocuments)){
                (data.participantDocuments).forEach(element => {
                    if(element.langId === AppConstants.LANGUAGE_ID_ENGLISH){
                        this.selectedTermsAndConditionsEn = new ParticipantDocuments();
                        this.selectedTermsAndConditionsEn.documentBase64_ = element.documentBase64_;
                        this.selectedTermsAndConditionsEn.contentType = element.contentType;
                        this.selectedTermsAndConditionsEn.documentName = element.documentName;
                        this.selectedTermsAndConditionsEn.documentTypeId = element.documentTypeId;
                        this.selectedTermsAndConditionsEn.langId = element.langId;
                        this.selectedTermsAndConditionsEn.participantId = element.participantId;
                        this.selectedTermsAndConditionsEn.participantDocumentId = element.participantDocumentId;
                        this.file_srcsTCen.push(this.selectedTermsAndConditionsEn.documentBase64_.valueOf());
                    }

                    if(element.langId === AppConstants.LANGUAGE_ID_PORTUGUESE){
                        this.selectedTermsAndConditionsPt = new ParticipantDocuments();
                        this.selectedTermsAndConditionsPt.documentBase64_ = element.documentBase64_;
                        this.selectedTermsAndConditionsPt.contentType = element.contentType;
                        this.selectedTermsAndConditionsPt.documentName = element.documentName;
                        this.selectedTermsAndConditionsPt.documentTypeId = element.documentTypeId;
                        this.selectedTermsAndConditionsPt.langId = element.langId;
                        this.selectedTermsAndConditionsPt.participantId = element.participantId;
                        this.selectedTermsAndConditionsPt.participantDocumentId = element.participantDocumentId;
                        this.file_srcsTCpt.push(this.selectedTermsAndConditionsEn.documentBase64_.valueOf());
                    }
                });
            }
            ...
            for (let i: number = 0; i < data.participantParams.length; i++) {
                switch (data.participantParams[i].paramId) {
                    ...
                    case 23:
                        ...
                        break;
                    case 24:
                        this.selectedItem.cashVoucherDebitText = data.participantParams[i].paramValue;
                        break;
                        case 25:
                            this.selectedItem.cashVoucherCreditText = data.participantParams[i].paramValue;
                            break;
                            case 26:
                                 
                                this.selectedItem.showTermsAndConditions = data.participantParams[i].paramValue == 'false' ? false : true;
                                break;
                    default:
                        break;
                }
    ...
    public setCompulsoryValidators() {
        ...
        (<any>this.myForm).controls.termsConditionsUpload.setValidators(null);
        (<any>this.myForm).controls.termsConditionsUpload.updateValueAndValidity();

        (<any>this.myForm).controls.termsConditionsUploadpt.setValidators(null);
        (<any>this.myForm).controls.termsConditionsUploadpt.updateValueAndValidity();

    }
    public FinalSave() {
        this.addParticipantDocuments();
        console.log(this.toJson())
        this.setCompulsoryValidators();
        this.compulsoryError = false;
        this.alertMessage = '';
    ...
    private addParticipantDocuments = () => {
        this.selectedItem.participantDocuments = [];
        if(AppUtility.isValidVariable(this.selectedTermsAndConditionsEn) && !AppUtility.isNullOrEmpty(this.selectedTermsAndConditionsEn.documentBase64_)) {
            this.selectedItem.participantDocuments.push(this.selectedTermsAndConditionsEn);
        }

        if(AppUtility.isValidVariable(this.selectedTermsAndConditionsPt) && !AppUtility.isNullOrEmpty(this.selectedTermsAndConditionsPt.documentBase64_)) {
            this.selectedItem.participantDocuments.push(this.selectedTermsAndConditionsPt);
        }
    }
    ...
    public checkMiscellaneousTab() {
        ...
        if ((this.selectedItem.clientControlAccount !== null && this.selectedItem.clientControlAccount.chartOfAccountId === null) || (this.selectedItem.clientControlAccount === null)) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('A Conta de Controle do Cliente deve ser selecionada \'<b>Aba Diversos</b>\'<br/>');
                this.dialogCmp.statusMsg = 'A Conta de Controle do Cliente deve ser selecionada Aba Diversos';
            }
            else {
                this.alertMessage = this.alertMessage.concat('Client Control Account should be selected \'<b>Miscellaneous Tab</b>\'<br/>');
                this.dialogCmp.statusMsg = 'Client Control Account should be selected on Miscellaneous Tab';
            }

            if (this.currentTab_ !== 'Miscellaneous') {
                this.dialogCmp.showAlartDialog('Error');
            }
        ...
        if (this.selectedItem.capitalGainAccount.chartOfAccountId != null && this.selectedItem.voucherType_Misc.voucherTypeId === null) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Tanto a Conta de Ganhos de Capital quanto o Tipo de Voucher são obrigatórios, se você selecionar em \'<b>Aba Diversos</b>\'<br/>');
            }
            else {
        ...
        if (!this.validDebitVoucherText || AppUtility.isNullOrEmpty(this.selectedItem.cashVoucherDebitText)) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Texto de débito do voucher em dinheiro inválido, se você selecionar uma das opções \'<b>Aba Diversos</b>\'<br/>');
            }
            else {
                this.alertMessage = this.alertMessage.concat('Cash Voucher Debit Text Invalid, if you select either on \'<b>Miscellaneous Tab</b>\'<br/>');
            }

            this.compulsoryError = true;
        }

        if (!this.validCreditVoucherText ||  AppUtility.isNullOrEmpty(this.selectedItem.cashVoucherCreditText)) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Texto de crédito do voucher em dinheiro inválido, se você selecionar uma das opções \'<b>Aba Diversos</b>\'<br/>');
            }
            else {
                this.alertMessage = this.alertMessage.concat('Cash Voucher Credit Text Invalid, if you select either on \'<b>Miscellaneous Tab</b>\'<br/>');
            }
    ...
    public checkContractNote() {
        ...
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba Nota Contratual</b>\'<br/>');
            }
            else {
    ...
    public checkStaionaryTab() {
        ...
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba Papelaria</b>\'<br/>');
            }
            else {
        ...
        if (this.file_srcsTCen.length == 0 || this.fileSizeExceedTCen) errorArray.push('Terms and Conditions language 1');
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba Papelaria</b>\'<br/>');
            }
            else {
                this.alertMessage = this.alertMessage.concat('Please upload ' + errorArray.join(", ") + ' on \'<b>Stationery Tab</b>\'<br/>');
            }
            this.compulsoryError = true;
        }

        if (this.file_srcsTCpt.length == 0 || this.fileSizeExceedTCpt) errorArray.push('Terms and Conditions language 2');
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba Papelaria</b>\'<br/>');
            }
    ...
    public checkFinancialsTab() {
        ...
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba Finanças</b>\'<br/>');
            }
            else {
    ...
    public checkEmailTab() {
        ...
        if (errorArray.length > 0) {
            if (this.lang === 'pt') {
                this.alertMessage = this.alertMessage.concat('Por favor, insira ' + errorArray.join(", ") + ' sobre \'<b>Aba E-mail</b>\'<br/>');
            }
            else {
    ...
    private addFromValidations() {
        this.myForm = this._fb.group({
            ...
            cashVoucherDebitText: [''],
            cashVoucherCreditText: ['']
    ...
    public downloadPDF = (langId) => {
          if(langId === AppConstants.LANGUAGE_ID_ENGLISH) {
            downloadAPI(this.selectedTermsAndConditionsEn.documentBase64_ , this.selectedTermsAndConditionsEn.documentName, this.selectedTermsAndConditionsEn.contentType);
          }
          else if(langId === AppConstants.LANGUAGE_ID_PORTUGUESE){
            downloadAPI(this.selectedTermsAndConditionsPt.documentBase64_ , this.selectedTermsAndConditionsPt.documentName, this.selectedTermsAndConditionsPt.contentType);
          }
    }
