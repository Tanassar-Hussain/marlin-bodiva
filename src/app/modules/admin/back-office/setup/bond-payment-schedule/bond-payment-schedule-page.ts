   ...
   **************************************/
                        ...
                        if (restData != null) {
                            restData.map((a) => {
                                if (a.marketType.description == "BOND" || a.marketType.description == "DEBT") {
                                    ...
                                    this.onSearchMarketChangeEvent(this.searchMarketId);
                                }
                            })
                        }
                        this.loader.hide();
    ...
    onView(): void {
        ...
        this.listingService.getPaymentScheduleForSecurity(this.searchSecurityId, this.generate).subscribe((restData) => {
            ...
            this.loader.hide();
            this.dialogCmp.statusMsg = AppConstants.SOMETHING_WENT_WRONG;
            this.dialogCmp.showAlartDialog('Error');
        });
    }
        ...
        this.listingService.getPaymentScheduleForSecurity(this.searchSecurityId, this.generate)
            .subscribe(
                (resData) => {
                    ...
                    this.loader.hide();
                    this.dialogCmp.statusMsg = AppConstants.MSG_PAYMENT_SCHEDULE_NOT_GENERATED;
                    this.dialogCmp.showAlartDialog('Error');
                });
    }
}