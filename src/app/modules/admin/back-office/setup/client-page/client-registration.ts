'use strict';
import { Component, OnInit, Inject, ViewEncapsulation, ViewChild, Input, EventEmitter, ElementRef, ChangeDetectorRef, Output } from '@angular/core';
...
import { SettlementMembers } from 'app/models/settlementMembers';
...
export class ClientRegistration implements OnInit {
  ...
  public isSubmittedTradConfig: boolean = false;
  ...
  @ViewChild(ClientPage) clientPage: ClientPage;
  ...
  public isDisabledTradConfig: boolean = false;
  ...
  clientStatus: any;
  isDisabledClientCode: boolean = false;
  currentDate: Date = new Date();
  disabledCreateAccount: boolean = false;
  ...
  constructor(private appState: AppState, private changeDetectorRef: ChangeDetectorRef, private listingService: ListingService, private _fb: FormBuilder,
    public userService: AuthService2, private translate: TranslateService, private splash: FuseLoaderScreenService, public router: ActivatedRoute, public route: Router, public datePipe: DatePipe) {
    ...
    this.isSubmittedTradConfig = false;
     ...
     this.listingService.getSettlementMembersFromCDS(data).subscribe({
       next : (data : any) => {
         if(AppUtility.isValidVariable(data) && !AppUtility.isEmptyArray(data)){
          this.settlementMembersList = data;
          var s: SettlementMembers = new SettlementMembers();
          s.Participant_Code = AppConstants.PLEASE_SELECT_VAL;
          s.Name = AppConstants.PLEASE_SELECT_STR;
            this.settlementMembersList.unshift(s);
             this.selectedItem.settlementMemberCode = this.settlementMembersList[0].Participant_Code;  
             this.selectedItem.eventSettlementMemberCode = this.settlementMembersList[0].Participant_Code;
          
         }else{
           this.settlementMembersList = [];
         }
 ...
 public onChangeClientCode(event){
     if(!AppUtility.isNullOrEmpty(event)){
          this.glCode_ = event;
     }
 }
 ...
 public onChangeName(){
    if(this.showCorporate){
       this.selectedItem.chartOfAccount.glDesc = this.selectedItem.contactDetail.companyName;
    }
    else{
      this.selectedItem.chartOfAccount.glDesc = this.selectedItem.contactDetail.firstName + " " + this.selectedItem.contactDetail.lastName;
    }
 }
  ...
  public onChnageVerifyDepo = (event) => {
  ...
  public updateBasicDetails = () => {
    ...
    if(!AppUtility.isValidVariable(this.selectedItem.eventSettlementMemberCode) || !AppUtility.isValidVariable(this.selectedItem.settlementMemberCode)){
          isValid = false;
          this.dialogCmp.statusMsg = AppConstants.MSG_ADD_SETTELEMENT_MEMBER;
  ...
  public onSaveDepository = () => {
    ...
    // 
    // this.selectedDepositoryItem.REF_No = this.selectedItem.clientId + "C-" + milliseconds;
    ...
    this.selectedDepositoryItem.Cash_Settlement_Agent_Code = this.selectedItem.settlementMemberCode;             //Added on 09-07-2024 ------- by Faizan
    this.selectedDepositoryItem.Event_Settlement_Agent_Code = this.selectedItem.eventSettlementMemberCode;             //Added on 15-08-2024 ------- by Faizan
    ...
    this.splash.show();
    this.listingService.invCDCRegister(this.selectedDepositoryItem).subscribe((data: any) => {
    ...
    //  this.changeStatusAfterSendDepository();   //Commented by Faizan ----- 19-07-23
      this.dialogCmp.statusMsg = AppConstants.MSG_ACCOUNT_REGISTRATION_SUBMIT;
      this.dialogCmp.showAlartDialog('LocalSuccess');
      this.splash.hide();

    }, (error) => {

      const wordsToPick = ["Account", "Code", ":" + this.selectedDepositoryItem.Account_Code];
      if (error.Message) {
        this.errorMessage = AppUtility.removeQuotesFromStartAndEndOfString(error.Message);
        const wordsArray = (this.errorMessage).split(" ");
        const specificWords = wordsArray.filter(word => wordsToPick.includes(word));
        const uniqueArray = specificWords.filter((item, index) => {
          return specificWords.indexOf(item) === index;
        });
        if (JSON.stringify(wordsToPick) === JSON.stringify(uniqueArray)) {
         // this.changeStatusSendDepositoryAlreadyExists();       //Commented by Faizan ----- 19-07-23
          this.dialogCmp.statusMsg = this.errorMessage;
          this.dialogCmp.showAlartDialog('LocalError');
          this.splash.hide();
        }
        else {
          this.dialogCmp.statusMsg = this.errorMessage;
        ...
        this.errorMessage = AppUtility.removeQuotesFromStartAndEndOfString(error);
        const wordsArray = (this.errorMessage).split(" ");
        const specificWords = wordsArray.filter(word => wordsToPick.includes(word));
        const uniqueArray = specificWords.filter((item, index) => {
          return specificWords.indexOf(item) === index;
  ...
  public changeStatusSendDepositoryAlreadyExists = () => {
    

    let x = {
      clientId: this.selectedItem.clientId,
      statusCode: AppConstants.INV_STATUS_REGISTERED,
    };
    this.listingService.updateClientStatusCodeForClients(x).subscribe(
      restData => {
      ...
      this.listingService.getClientById(AppConstants.participantId, invClientId).subscribe(
        restData => {
          ...
          if (AppUtility.isValidVariable(restData.clientId)) {
            ...
            this.clientStatus = restData.statusCode;
            if (this.clientStatus === AppConstants.INV_STATUS_ACTIVE) {
              this.isDisabledClientCode = true;
            }
            else {
              this.isDisabledClientCode = false;
            }
  ...
  public clearFields() {
    ...
    this.selectedItem.clientCode = AppConstants.PARTICPANT_BANK_CODE;
  ...
  public fileChangeEvent(fileInput: any) {
    ...
    if (this.fileName_.length > 0) {
      ...
      if (!blnValid) {
        if (this.lang === 'pt') {
          this.dialogCmp.statusMsg = "Desculpa, " + this.fileName_ + " é inválido, as extensões permitidas são: " + _validFileExtensions.join(", ");
        }
        else {
  ...
  public onCreateLocalAccount = () => {
    this.isSubmittedTradConfig = true;  

   if(this.selectedItem.agreeTermsAndConditions === false && this.participantsList[0]?.displayTermsAndConditions == true){
    if (this.lang === 'pt') {
      this.errorMessage = "Mark concordou com os Termos e Condições na aba básica."
      this.dialogCmp.statusMsg = this.errorMessage;
      this.dialogCmp.showAlartDialog('Notification');
      return;
    } else {
      this.errorMessage = "Mark agreed Terms and Conditions on basic tab."
      this.dialogCmp.statusMsg = this.errorMessage;
      this.dialogCmp.showAlartDialog('Notification');
      return;
    }
    ...
    if (AppUtility.isValidVariable(this.selectedItem.contactDetail.identificationType)
      && !(AppUtility.isEmpty(this.itemsBeneficiaryList) || this.itemsBeneficiaryList.itemCount == 0)) {
      let isReturn: boolean = false;
      this.itemsBeneficiaryList.items.forEach(element => {
        if (element.beneficiaryCNIC === this.selectedItem.contactDetail.identificationType) {
          if (this.lang === 'pt') {
            this.errorMessage = "O Número de Identificação e o Número de Identificação do Beneficiário não devem ser iguais."
            this.dialogCmp.statusMsg = this.errorMessage;
            this.dialogCmp.showAlartDialog('Error');
            isReturn = true;
          } else {
            this.errorMessage = "Identification number and beneficiary's identification number should not be same."
            this.dialogCmp.statusMsg = this.errorMessage;
            this.dialogCmp.showAlartDialog('Error');
            isReturn = true;
          }
    ...
    if (this.showJoint) {
      if (AppUtility.isValidVariable(this.selectedItem.contactDetail.identificationType)
        && !(AppUtility.isEmpty(this.itemsJointAccountList) || this.itemsJointAccountList.itemCount == 0)) {
        let isReturn: boolean = false;
        this.itemsJointAccountList.items.forEach(element => {
          if (element.contactDetail.identificationType === this.selectedItem.contactDetail.identificationType) {
            if (this.lang === 'pt') {
              this.errorMessage = "O número de identificação e o número de identificação da conta conjunta não devem ser iguais."
              this.dialogCmp.statusMsg = this.errorMessage;
              this.dialogCmp.showAlartDialog('Error');
              isReturn = true;
            } else {
              this.errorMessage = "Identification number and joint account's identification number should not be same."
              this.dialogCmp.statusMsg = this.errorMessage;
              this.dialogCmp.showAlartDialog('Error');
              isReturn = true;
            }
    ...
    this.selectedItem.user = null;
    if (AppUtility.isValidVariable(this.selectedItem.incomeSource) && this.selectedItem.incomeSource.incomeSourceId === null) {
      this.selectedItem.incomeSource = null;
    }
    ...
    this.selectedItem.statusCode = AppConstants.INV_ACTIVE;
    ...
    this.selectedItem.clientExchange[0].active = true;
    this.selectedItem.clientExchange[0].depositWorth = this.selectedClientExchange.depositWorth;
    ...
    if (!AppUtility.isEmpty(this.currentTab_)) {
      ...
      if (this.currentTab_ != "BI" && isValid == false) {
        if (this.lang === 'pt') {
          this.dialogCmp.statusMsg = 'Por favor, preencha todos os campos obrigatórios da aba Básico.';
        }
        else {
      ...
      if (this.currentTab_ != "CON" && isValid == false) {
        if (this.lang === 'pt') {
          this.dialogCmp.statusMsg = 'Por favor, preencha todos os campos obrigatórios da aba Contato.';
        }
        else {
    ...
    this.disabledCreateAccount = true;
    this.splash.show();
    this.listingService.saveClient(this.selectedItem).subscribe((restData) => {
      ...
      this.disabledCreateAccount = false;
    }, (error) => {
      this.splash.hide();
      if (error.message) {
        this.errorMessage = error.message;
  ...
  public onCreateAccount = () => {
    ...
    // this.selectedItem.statusCode = AppConstants.INV_STATUS_ACTIVE;
    this.splash.show();
      ...
      if (isValid) {
        if (this.isEditingClientExchange) {
          ...
          for (let i = 0; i < this.itemsClientExchangeList.itemCount; i++) {
            if (this.selectedClientExchange.exchangeId == this.itemsClientExchangeList.items[i].exchangeId) {
              if (this.lang === 'pt') {
                this.dialogCmp.statusMsg = this.itemsClientExchangeList.items[i].exchangeName + " já existe.";
              }
              else {
          ...
          cba.depositWorth = this.selectedClientExchange.depositWorth;
          ...
          if (this.lang === 'pt') {
            this.dialogCmp.statusMsg = "Registro adicionado com sucesso";
          }
          else {
  ...
  public onAddDocument(model: any, isValid: boolean) {
    ...
    if (isValid) {
      if (!AppUtility.isEmpty(this.itemsDocumentList) && this.itemsDocumentList.itemCount == 5) {
        if (this.lang === 'pt') {
          this.dialogCmp.statusMsg = "São permitidos no máximo 5 documentos.";
        }
        else {
  ...
  public onSaveBankDetailAction(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      if(this.itemsBanAccountList.items.length === 1){
        if (this.lang === 'pt')
        this.dialogCmp.statusMsg = 'Apenas 1 banco é permitido.';
      else
        this.dialogCmp.statusMsg = 'Only 1 bank is allowed.';

      this.dialogCmp.showAlartDialog('Error');
      return false;
      }

      if (this.itemsBanAccountList.items.some((element) => element.bankAccountNo === this.selectedBankAccountItem.bankAccountNo)) {
        if (this.lang === 'pt')
          this.dialogCmp.statusMsg = 'Números de contas bancárias duplicados não são permitidos.';
        else
          this.dialogCmp.statusMsg = 'Duplicate bank account numbers not allowed.';
  ...
  public onSaveBeneficiaryAction(model: any, isValid: boolean) {
    ...
    if (AppUtility.isEmpty(this.selectedBeneficiaryItem.beneficiaryName) ||
      ...
      ((this.idTypeBeneficiaryExactLength !== null && this.idTypeBeneficiaryMinLength !== 1 && this.beneficiaryForm.get('beneficiaryCNIC').value.length !== 0) && (this.beneficiaryForm.get('beneficiaryCNIC').value.length < this.idTypeBeneficiaryExactLength || this.beneficiaryForm.get('beneficiaryCNIC').value.length > this.idTypeBeneficiaryExactLength)) ||
      ((this.idTypeBeneficiaryExactLength !== null && this.idTypeBeneficiaryMinLength === 1) && (this.beneficiaryForm.get('beneficiaryCNIC').value.length > this.idTypeBeneficiaryExactLength))
      ) {
      return false;
    }
  ...
  private fillClientFromJson(c: Client, data: any) {
    ...
    c.accountTypeNew.accTypeId = data.accountTypeNew.accTypeId; //Added New --- 22-03-2023
    // c.accountTypeNew.accTypeId = data.accountType;  //Commented By Faizan --- 22-03-2023
    c.clientId = data.clientId;
    c.statusCode = data.statusCode;
    c.sentToDepo = data.sentToDepo;
    c.agreeTermsAndConditions = data.agreeTermsAndConditions;
    if(c.agreeTermsAndConditions === true && c.statusCode === 'AC'){
      this.myForm.get('agreeTC').disable();
     }else{
      this.myForm.get('agreeTC').enable();
     }
     
    setTimeout(() => {
      c.settlementMemberCode = data.settlementMemberCode;
      c.eventSettlementMemberCode = data.eventSettlementMemberCode;
    }, 500);
  ...
  private fillClientExchangeFromJSON(c: ClientExchange, data: any) {
    ...
    c.margin = data.margin;
    c.openPositionStatus = data.openPositionStatus;
    c.depositWorth = data.depositWorth;
    c.clientId = data.clientId;
  ...
  private populateParticipants() {
    ...
    this.listingService.getParticipantListByExchagne(AppConstants.exchangeId).subscribe(restData => {
      ...
      if (AppUtility.isEmpty(restData)) {
        ...
        setTimeout(() => {
          this.selectedItem.participant.participantId = AppConstants.participantId;
          if(this.participantsList[0].displayTermsAndConditions === true){
            this.showTermsAndConditions = true;
           }else{
               this.showTermsAndConditions = false;
           }
  ...
  private loadClientMarkets() {
    ...
    this.clientMarket = [];
    if (AppUtility.isEmpty(this.selectedItem.clientId)) {
       if(this.selectedItem.verifyDepo === true){
       this.itemsAllowedMarketList = new wjcCore.CollectionView();
       }
    } else {
  ...
  private validateBasicInfo(): boolean {
    ...
    if(this.selectedItem.agreeTermsAndConditions === false && this.participantsList[0]?.displayTermsAndConditions == true){
      if (this.lang === 'pt') {
        this.errorMessage = "Mark concordou com os Termos e Condições."
        this.dialogCmp.statusMsg = this.errorMessage;
        this.dialogCmp.showAlartDialog('Notification');
        return;
      } else {
        this.errorMessage = "Mark agreed Terms and Conditions."
        this.dialogCmp.statusMsg = this.errorMessage;
        this.dialogCmp.showAlartDialog('Notification');
        return;
      }
    ...
    if (this.showIndividual && this.showTrustee) {
      if (AppUtility.isEmpty(this.selectedItem.contactDetail.firstName) ||
        ...
        (AppUtility.isEmpty(this.selectedItem.settlementMemberCode)) ||
        (AppUtility.isEmpty(this.selectedItem.eventSettlementMemberCode)) ) {
        return false;
      }
      else {
        this.selectedItem.contactDetail.companyName = "";
  ...
  private validateJointAccount(): boolean {
    if (this.showJoint) {
      if (AppUtility.isEmpty(this.itemsJointAccountList) || this.itemsJointAccountList.itemCount == 0) {
        if (this.lang === 'pt') {
          this.dialogCmp.statusMsg = 'Por favor, adicione algumas informações da conta na guia Joint.';
        }
        else {
  ...
  private validateBeneficiary(): boolean {
    ...
    if ((AppUtility.isEmpty(this.itemsBeneficiaryList) || this.itemsBeneficiaryList.itemCount == 0) && (this.showIndividual || this.showJoint)) {
      if (this.lang === 'pt') {
        this.dialogCmp.statusMsg = 'Por favor, adicione alguns registros na guia Beneficiário.';
      }
      else {
  ...
  private validateBankAccount(): boolean {
    if (AppUtility.isEmpty(this.itemsBanAccountList) || this.itemsBanAccountList.itemCount == 0) {
      if (this.lang === 'pt') {
        this.dialogCmp.statusMsg = 'Por favor, adicione alguns registros na guia Banco.';
      }
      else {
  ...
  private validateDocument(): boolean {
    ...
    if (AppUtility.isEmpty(this.itemsDocumentList) || this.itemsDocumentList.itemCount == 0) {
      if (this.lang === 'pt') {
        this.dialogCmp.statusMsg = 'Adicione alguns documentos na guia Documento.';
      }
      else {
  ...
  private addFromValidations() {
    this.myForm = this._fb.group({
      ...
      client_code: ['', Validators.compose([Validators.required, this.removeSpaces])],
      todayDate: [''],
      participant: [null, Validators.compose([Validators.required])],
      account_type: [null, Validators.compose([Validators.required])],
      tax_no: ['', Validators.compose([Validators.required, this.removeSpaces])],
      trustee: ['', Validators.compose([Validators.required, this.removeSpaces])],
      contactPerson: ['', Validators.compose([Validators.required, this.removeSpaces])],
      registerationDate: [''],
      identification_type: ['', Validators.compose([Validators.required])],
      identification_no: ['', Validators.compose([Validators.required, this.removeSpaces])],
      agreeTC : [''],

      //////////////////////////////////
      glCode: ['', Validators.compose([Validators.required, this.removeSpaces])],
      coaCode: [''],
      coaDesc: ['', Validators.compose([Validators.required, this.removeSpaces])],
      searchClientName: [''],
      searchClientCode: [''],
      active: [''],
      commissionSlab: [''],
      ...
      firstName: ['', Validators.compose([Validators.required, Validators.pattern(/^[^~`!@#$%^*()-+=|\{}':;.<>/?\/]*$/),  this.removeSpaces])],
      lastName: ['', Validators.compose([Validators.required, Validators.pattern(/^[^~`!@#$%^*()-+=|\{}':;.<>/?\/]*$/), this.removeSpaces])],
      ...
      registrationNo: ['', Validators.compose([Validators.required, Validators.pattern(AppConstants.validatePatternString), this.removeSpaces])],
      dob: [''],
      ...
      incomePercentage: [''],
      verifyDepo : [''],
      settlementMember : ['' , Validators.compose([Validators.required])],
      eventSettlementMember : ['' , Validators.compose([Validators.required])],
    });
    ...
    this.bankDetailForm = this._fb.group({
      ...
      accountNo: ['', Validators.compose([Validators.required, Validators.pattern(AppConstants.validatePatternString), this.removeSpaces])],
      accountTitle: ['', Validators.compose([Validators.required, this.removeSpaces])],
    });

    this.beneficiaryForm = this._fb.group({
      beneficiary_tax_no: ['', Validators.compose([Validators.required, this.removeSpaces])],
      relationId: ['', Validators.compose([Validators.required])],
      beneficiaryName: ['', Validators.compose([Validators.required, this.removeSpaces])],
      beneficiaryCNIC: ['', Validators.compose([Validators.required ,this.removeSpaces])],
      identificationTypeId: ['', Validators.compose([Validators.required])],
    });

    this.jointAccountForm = this._fb.group({
      tax_no: ['', Validators.compose([Validators.required, this.removeSpaces])],
      firstName: ['', Validators.compose([Validators.required, Validators.pattern(AppConstants.validatePatternString), this.removeSpaces])],
      lastName: ['', Validators.compose([Validators.required, Validators.pattern(AppConstants.validatePatternString), this.removeSpaces])],
      identificationTypeId: ['', Validators.compose([Validators.required])],
      identificationNumber: ['', Validators.compose([Validators.required, this.removeSpaces])],
  ...
  public getNotification(btnClicked) {
    if (btnClicked == 'Success' || btnClicked === 'LocalError') {
      this.notifyParent.emit('Refresh Grid');
    }
