...
export class AuditTrailDashboardComponent implements OnInit {
    ...
    constructor(private appState: AppState, private authService: AuthService, private orderSvc: OrderService, private listingSvc: ListingService,
        ...
        private splash: FuseLoaderScreenService, private dashboardService: DashboardService, 
        private toast: ToastrService, private _changeDetectorRef: ChangeDetectorRef,) {
    ...
    updateData(eventLog): void {
        ...
        for (let i = 0; i < orders.length; i++) {
            if (orders[i].order_state !== 'partial_filled' && orders[i].order_state !== 'filled') {
                ...
                if (this.selectedAssetClass !== AppConstants.ASSET_CLASS_BONDS) {
                    ...
                    orders[i].settlement_amount = (p) * orders[i].volume;
                }
        ...
        trades.map(res => {
            res.state_time = new Date(res.execution_time + 'Z');
        ...
        for (let j = 0; j < trades.length; j++) {
            ...
            // orders[j].state_time = new Date(orders[j].execution_time + 'Z');
            // orders[j].execution_time = new Date(orders[j].execution_time + 'Z');
