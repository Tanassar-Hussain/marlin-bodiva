...
export class NegotiatedETFDealsListComponent {
    ...
    data: any = [];
    data_temp: any = [];
    assetclass = AppConstants.ASSET_CLASS_ID_ETFS
    modal = true;
    statusMsg: string;
    cancleRecord: any;
    ...
    constructor(private translate: TranslateService, public authService: AuthService2, private orderSvc: OrderService,
        private splash: FuseLoaderScreenService, private listingSvc: ListingService, private toast: ToastrService, public authServiceOMS: AuthService,) {
    ...
    updateData(eventLog): void {
        ...
        let forMe = []
        this.data.map(a => {
            if (a.order_type == 'negotiated' && a.asset_id == AppConstants.ASSET_CLASS_ID_ETFS) {
                ...
                a.assetClass = AppConstants.ASSET_CODE_ETF
    ...
    cancelOrder(payLoad) {
        ...
        if (payLoad.negotiated_order_state === 'Initiate') {
            ...
            selectedOrder.value = Number(selectedOrder.volume) * Number(selectedOrder.price);
            selectedOrder.settlementValue = Number(selectedOrder.volume) * Number(selectedOrder.price);
            ...
            this.orderCancel.order.sender_username = AppConstants.username;
