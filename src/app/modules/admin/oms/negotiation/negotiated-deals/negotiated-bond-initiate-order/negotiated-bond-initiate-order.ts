...
export class NegotiatedBondInitiateOrderComponent {
  ...
  constructor(private appState: AppState, public authService: AuthService2, public authServiceOMS: AuthService, private dataService: DataServiceOMS,
    ...
    private _fb: FormBuilder, private translate: TranslateService, public appUtility2: AppUtility,
    public socket: WebSocketService, public shareOrderService: ShareOrderService,
  ...
  public onChangeClient = (event) => {
    ...
    if(AppUtility.isValidVariable(event)) this.order.account = event;
  ...
  public refreshBuyingPower = () => {
    if (AppUtility.isValidVariable(this.order.account)) {
  ...
  private _submitClientOrder(){
    const order = {
        ...
        broker: AppUtility.isNullOrEmpty(this.selectedClient.participantCode) ? AppConstants.participantCode : this.selectedClient.participantCode,
    ...
    this.listingService.submitClientNegOrder(order).subscribe({
        next: (res) => {
        ...
        error: (error) => {
            ...
            this.appState.showLoader = false;
            let alertMessage: AlertMessage = new AlertMessage();
            alertMessage.message = error || 'Something went wrong!';
