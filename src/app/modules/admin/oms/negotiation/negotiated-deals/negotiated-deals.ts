...
import { NegotiatedETFInitiateOrderComponent } from './negotiated-etf-initiate-order/negotiated-etf-initiate-order';
import { NegotiatedETFAcceptOrderComponent } from './negotiated-etf-accept-order/negotiated-etf-accept-order';
...
export class NegotiatedDealsComponent implements OnInit, AfterViewInit {
   ...
   public selectedAssetClass: string = '';
   ...
   @ViewChild(NegotiatedETFInitiateOrderComponent) negotiatiedETFOrder: NegotiatedETFInitiateOrderComponent;

   @ViewChild(NegotiatedEquityAcceptOrderComponent) negotiatedEquityAcceptOrder: NegotiatedEquityAcceptOrderComponent;
   @ViewChild(NegotiatedBondAcceptOrderComponent) negotiatedBondAcceptOrder: NegotiatedBondAcceptOrderComponent;
   @ViewChild(NegotiatedETFAcceptOrderComponent) negotiatedETFAcceptOrder: NegotiatedETFAcceptOrderComponent;
   ...
   public initiatedOrder = (state, selectedRow, assetClass) => {
      ...
      if (typeof (this.parentSelectedRow.volume) === 'number') {
         this.parentSelectedRow.volume = this.parentSelectedRow.volume.toString()
     }
      ...
      if (assetClass == AppConstants.ASSET_CLASS_ID_EQUITIES) {
         ...
         jQuery('#negotiated-bond-accpet').modal({ backdrop: 'static', keyboard: true });
         jQuery('#negotiated-bond-accpet').modal('show');
      }
      else if (assetClass == AppConstants.ASSET_CLASS_ID_ETFS) {
         this.selectedAssetClass = AppConstants.ASSET_CLASS_ETF;

         jQuery('#negotiated-etf-accpet').modal({ backdrop: 'static', keyboard: true });
         jQuery('#negotiated-etf-accpet').modal('show');
      }
      else {
         this.selectedAssetClass = AppConstants.ASSET_CLASS_EQUITIES;
      }
   ...
   onClose() {
      ...
      if (this.orderState === 'new') {
         if (AppConstants.selectedAssetClass === AppConstants.ASSET_CLASS_EQUITIES) {
            ...
            AppConstants.selectedAssetClass = AppConstants.ASSET_CLASS_EQUITIES;
            this.negotiatiedETFOrder.init();
            jQuery('#new-negotiated-order-all-menu').modal('hide');
         }
      }


      else if (this.orderState === 'Initiate' || this.orderState === 'Reject') {
         
         if (this.selectedAssetClass === AppConstants.ASSET_CLASS_EQUITIES) {
            this.selectedAssetClass = AppConstants.ASSET_CLASS_EQUITIES;
            // this.negotiatiedEquityAcceptOrder.init();
           
            this.negotiatedEquityAcceptOrder.onClose();
            ...
            this.negotiatedBondAcceptOrder.onClose();
            ...
            this.negotiatedETFAcceptOrder.onClose();
            // this.negotiatedBondAcceptOrder.init();
            // jQuery('#negotiated-bond-accpet').modal('hide');
         }
