...
@Component({
  selector: '[negotiated-etf-accept-order]',
  templateUrl: './negotiated-etf-accept-order.html',
  encapsulation: ViewEncapsulation.None,
})

export class NegotiatedETFAcceptOrderComponent {
  ...
  onClose() {
    ...
    jQuery('#negotiated-etf-accpet').modal('hide');
  ...
  updateSymbolList(data) {
    ...
    if (AppUtility.isValidVariable(data) && !AppUtility.isEmpty(data)) {
      for (let i = 0; i < data.length; i++) {
        if (data[i].marketTypeCode === AppConstants.MARKET_TYPE_ETF_) {
