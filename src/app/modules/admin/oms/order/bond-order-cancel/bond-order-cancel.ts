...
export class BondOrderCancel implements OnInit, AfterViewInit
{
  ...
  updateSecurityDetails(data)
  {
   ...
   if(AppUtility.isValidVariable(this.securityMarketDetails.bondSubCategory)){
  ...
  submitOrder()
  {
    if (this.cancelOrder.order.settlementValue === undefined || this.cancelOrder.order.settlementValue === null ) {      
      this.cancelOrder.order.settlementValue = 0  ; 
    }
