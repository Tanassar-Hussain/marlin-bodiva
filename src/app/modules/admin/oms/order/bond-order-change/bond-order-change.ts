...
export class BondOrderChange implements OnInit, AfterViewInit {
  ...
  constructor(private appState: AppState, public authService: AuthService2, public authServiceOMS: AuthService, private listingService: ListingService,
    private dataService: DataServiceOMS, private orderService: OrderService, private _pendingOrderService : PendingOrdersService,
    private _fb: FormBuilder, private translate: TranslateService, public cdr: ChangeDetectorRef,) {
  ...
  updateOrder(orderResponse): void {
    ...
    if (isNaN(this.order.yield) || this.order.yield === null) {
      this.order.yield = 0;
    }
  ...
  updateSecurityDetails(data) {
    ...
    this.securityMarketDetails.accrudeProfit = this.order.accrudeProfit;
    this.order.settlementValueCurrency = this.order.settlementValue * Number(this.securityMarketDetails.currencyRate);
  }
  ...
  submitOrder() {
    ...
    this.appState.showLoader = true;
    if (this.changeOrder.order.settlementValue === undefined || this.changeOrder.order.settlementValue === null || Number.isNaN(this.changeOrder.order.settlementValue)) {
      this.changeOrder.order.settlementValue = 0;
      this.changeOrder.order.settlementValueCurrency = 0;
    }
