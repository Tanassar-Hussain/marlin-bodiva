...
import { PaymentScheduleComponent } from './payment schedule/payment-schedule';
...
export class BondOrderNew implements OnInit, AfterViewInit, OnChanges {
  ...
  public tradeType;
  public vTradePayload: virtualOrder;

  @ViewChild('orderSubmittedDlg', { static: false }) orderSubmittedDlg: wjcInput.Popup;
  @ViewChild('paymentScheduleModel', { static: false }) paymentScheduleModel: wjcInput.Popup;
  ...
  holding: number = 0;
  bondPaymentScheduleDisabled: boolean = true;
  symbolCodeForPaymentSchedule: string = '';
  viewSchedule: boolean = false;
  disabledSubmit: boolean = false;
  loggedInUserType: string = AppConstants.userType;
  public userType : any = UserTypes;
  ...
  ngOnInit() {
    ...
    if (this.loggedInUserType === AppConstants.USER_TYPE_PARTICIPANT_CODE || this.loggedInUserType === AppConstants.USER_TYPE_PARTICIPANT_ADMIN_CODE) {
      this.loadTraders();
    }
    else {
      this.traders = [];
  ...
  updateSymbolList(data) {
    ...
    if (AppUtility.isValidVariable(data) && !AppUtility.isEmpty(data)) {
      for (let i = 0; i < data.length; i++) {
        if ((data[i].marketTypeCode === AppConstants.MARKET_TYPE_BOND) || (data[i].marketTypeCode === AppConstants.MARKET_TYPE_QUOTE_ && data[i].securityType === AppConstants.SECURITY_TYPE_BONDS)) {
...
getLoggedInUserClientsList() {
  ...
  submitOrder() {
   ...
   else 
   {
    ...
    if (this.tradeType === AppConstants.VIRTUAL_TRADE_TYPE) {
      ...
      this.vTradePayload.tradeValue = this.order.settlementValue;
      this.vTradePayload.yield = this.order.yield;
  ...
  updateSecurityDetails(data) {
    ...
    let couponType = this.securityMarketDetails.fisDetail.bondType.bondType.toUpperCase();

    if (this.securityMarketDetails.fisDetail.bondType.bondType.toUpperCase() != "ZERO COUPON" && this.order.symbolMktExch != '') {
      this.bondPaymentScheduleDisabled = false;
    }
    else
      this.bondPaymentScheduleDisabled = true;
  ...
  splitSymbolExchMkt() {
    ...
    if (this.cmbSymbol.selectedValue != null && this.cmbSymbol.selectedValue.length > 0) {
      ...
      //For Child
      this.symbolCodeForPaymentSchedule = this.order.symbol;
  ...
  onSymbolChange(): void {
    ...
    // this.bondPaymentScheduleDisabled = this.order.symbolMktExch != '' ? false : true
  }
  ...
  updateSettlementAmount() {
    ...
    if (Number(this.inputVolume.value) == 0 || Number(this.inputPrice.value) == 0) {
      this.order.settlementValue = 0
      this.order.settlementValueCurrency = 0;
      this.order.tempSettlementAmount  = 0;
    }
      ...
      this.order.tempSettlementAmount = this.order.settlementValue;
    }
  ...
  updateSettlementAmountMarketType(price : number) {
    ...
    if (price == 0) {
      ...
      if (this.securityMarketDetails != null && this.securityMarketDetails.fisDetail != null
        ...
        && this.securityMarketDetails.fisDetail.bondType.bondTypeId != 0) {
        if (this.isBondPricingMechanismPercentage)
          this.order.value = (Number(this.inputVolume.value) * this.securityMarketDetails.parValue) * (Number(price) / 100);
        else
          this.order.value = (Number(this.inputVolume.value) * this.securityMarketDetails.parValue) * (Number(price));
      } else {
        this.order.value = Number(this.inputVolume.value) * Number(price);
      }
      this.order.tempSettlementAmount = Number(this.order.value) + Number(this.securityMarketDetails.accrudeProfit);
  ...
  onPriceChangeMarketType(price : number): void {
    this.updateSettlementAmountMarketType(price);
  }
  ...
  onClickPaymentSchedule() {
    ...
    this.paymentScheduleModel.isDraggable = true;
    this.showDialog(this.paymentScheduleModel);
  }

  onCloseSchedule() {
    this.viewSchedule = false;
    this.paymentScheduleModel.hide(true);
    this.cmbSymbol.focus();
  }
  // ...........................Payment Schedule..................................
}
