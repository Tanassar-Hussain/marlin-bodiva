...
import { EtfOrderChange } from '../etf-order-change';
...
export class ChangeOrderAll implements OnInit, AfterViewInit {
   ...
   @ViewChild(EtfOrderChange) changeOrderETF : EtfOrderChange;

   @ViewChild('changeEquity') changeEquity :ElementRef;
   @ViewChild('changeBonds') changeBonds: ElementRef;
   @ViewChild('changeETFs') changeETFs : ElementRef;
   ...
   public selectedAssetClass : string =  AppConstants.ASSET_CLASS_EQUITIES;
    ...
    tradeType: string;
    actualTrade : String = AppConstants.ACTUAL_TRADE_TYPE;
    virtualTrde : String = AppConstants.VIRTUAL_TRADE_TYPE;

    assetClassEquities : String = AppConstants.ASSET_CLASS_EQUITIES;
    assetClassBonds : String = AppConstants.ASSET_CLASS_BONDS;
    assetClassETFs : String = AppConstants.ASSET_CLASS_ETF;
    ...
    constructor(private appState: AppState, public authService: AuthService2, private dataService: DataServiceOMS,
      ...
      this.assetClassEquities = AppConstants.ASSET_CLASS_EQUITIES;
      this.assetClassBonds = AppConstants.ASSET_CLASS_BONDS;
      this.assetClassETFs = AppConstants.ASSET_CLASS_ETF;
...
public clickEquityTab=()=>{
 setTimeout(() => {
  let inputElement: HTMLElement = this.changeEquity.nativeElement as HTMLElement;
  inputElement.click();
...
public clickBondsTab=()=>{
 setTimeout(() => {
  let inputElement: HTMLElement = this.changeBonds.nativeElement as HTMLElement;
  inputElement.click();
...
public clickETFsTab=()=>{
  setTimeout(() => {
   let inputElement: HTMLElement = this.changeETFs.nativeElement as HTMLElement;
   inputElement.click();
    ...
    public show=()=> {
        ...
        this.auth2Service.tradeChange.next(this.tradeType);
          ...
          this.changeOrderEquity.orderNo.focus();
          this.changeOrderBonds.orderNo.focus();
    ...
    onClose() {
        ...
        else if(AppConstants.selectedAssetClass == AppConstants.ASSET_CLASS_ETF)
        {
    ...
    public loadOrderNo(orderNo, user: string = AppConstants.username)
    {
       ...
       //ETFs Change Order
       public getOrderNoETFs(orderNo, user: string = AppConstants.username)
       {
           this.selectedAssetClass = AppConstants.ASSET_CLASS_ETF;
           this.changeOrderETF.loadOrderNo(orderNo , user);            
       }
