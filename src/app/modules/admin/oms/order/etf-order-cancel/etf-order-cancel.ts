...
@Component({
    selector: 'etf-order-cancel',
    templateUrl: './etf-order-cancel.html',
    styleUrls: ['../components.style.scss'],
    encapsulation: ViewEncapsulation.None,
  })
export class EtfOrderCancel implements OnInit, AfterViewInit {
  ...
  securityMarketDetails: SecurityMarketDetails = new SecurityMarketDetails();
    ...
    updateOrder(orderResponse): void
    {
      ...
      if (this.modalId !== 'cancel_dialogBox' && this.modalId !== 'cancel_dialogModalWO')
      {
        ...
        let marketTypeCode = this.dataService.getMarketType(orderResponse.order[0].exchange, orderResponse.order[0].market, orderResponse.order[0].symbol);
