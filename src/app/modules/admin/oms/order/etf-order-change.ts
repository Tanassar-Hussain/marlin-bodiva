...
@Component({
    selector: 'etf-order-change',
    templateUrl: './etf-order-change.html',
    styleUrls: ['./components.style.scss'],
    encapsulation: ViewEncapsulation.None,
  })
export class EtfOrderChange implements OnInit, AfterViewInit {
    ...
    onInitialized = new EventEmitter<EtfOrderChange>();
   ...
   securityMarketDetails = new SecurityMarketDetails();
    ...
    constructor(private appState: AppState, public authService: AuthService2, public authServiceOMS : AuthService ,private dataService: DataServiceOMS,
                private listingService: ListingService, private orderService: OrderService, private _pendingOrderService : PendingOrdersService,
    ...
    updateOrder(orderResponse): void
    {
      ...
      if ( this.modalId !== 'edit_dialogBox' && this.modalId !== 'edit_dialogModalWO' )
      {
        ...
        if (marketTypeCode !== AppConstants.MARKET_TYPE_ETF_)
          return;
      }
      ...
      this.order.settlementValueCurrency = this.order.value *  Number(this.securityMarketDetails.currencyRate);
    ...
    //  this.securityMarketDetails = new SecurityMarketDetails();
      if (this.securityMarketDetails.exchangeId > 0 && this.securityMarketDetails.marketId > 0 && this.securityMarketDetails.securityId > 0) {
        this.appState.showLoader = true;
        this.listingService.getSymbolMarket(this.securityMarketDetails.exchangeId, this.securityMarketDetails.marketId, this.securityMarketDetails.securityId)
          .subscribe(data => {
    ...
    submitOrder()
    {
      ...
      this.orderService.changeOrder(this.changeOrder).subscribe(
        ...
        this._pendingOrderService.changeWorkingOrders(this.changeOrder).subscribe((res : any) => {
          this.disabledSubmit = false;
          this._createAlert('success', res.message);
          this.showOrderConfirmationMsg();
         }, (err : any)=> {
          this.disabledSubmit = false;
          this._createAlert('danger', err.message);
          this.showOrderConfirmationMsg();
         });
      }
