import { Component, Inject, ViewEncapsulation, ViewChild, Input, Output, EventEmitter, AfterViewInit, OnInit, OnDestroy } from '@angular/core';
...
export class RepoOrderListComponent implements AfterViewInit, OnInit, OnDestroy{
...
constructor(private translate: TranslateService, private splash: FuseLoaderScreenService, private listingSvc: ListingService, 
...
public updateorderConfirmation = (data) => {
    if(data.market === AppConstants.MARKET_TYPE_REPO_ && this.router.url === '/repo/repo-order-list'){
        if (this.userType === AppConstants.USER_TYPE_PARTICIPANT_CODE || this.userType === AppConstants.USER_TYPE_PARTICIPANT_ADMIN_CODE) {
            this.loadTraders();
        }
        else
...
updateData(eventLog): void {
    ...
    for (let i = 0; i < orders.length; i++) {
        ...
        if (orders[i].order_state !== 'partial_filled' && orders[i].order_state !== 'filled' && orders[i].repo_leg === 1) {
   ...
   if(AppUtility.isValidVariable(forMe) &&!AppUtility.isEmptyArray(forMe)){
      forMe.map((element : any) => {
         if(element.repo_leg === 1 && element.repo_type === 'BuyBack'){
            element.counter_side = 'buy';
         }else if(element.repo_leg === 1 && element.repo_type === 'SellBack'){
...
public getEventLogSingle = (orderNo : Number) => {
    this.orderSvc.getSingleEventLog(orderNo).subscribe(
        data => {
             
            this.splash.hide();
            this.updateSingleRecordData(data);
        },
        error => {
            this.splash.hide();
            this.errorMsg = <any>error;
...
updateSingleRecordData(eventLog): void {
    // this.data = [];
    // this.data_temp = [];
    ...
    this.data = uniqueData
     ...
     let byMe = this.dataInitiatedByMe;
     let forMe = this.dataInitiatedForMe;
    this.data.map(a => {
        if (a.order_type == 'negotiated' && a.asset_id == AppConstants.ASSET_CLASS_ID_BONDS) {
    ...
    this.dataInitiatedByMe = byMe;
    this.dataInitiatedByMe = this.dataInitiatedByMe.slice();
   ...
   if(AppUtility.isValidVariable(forMe) &&!AppUtility.isEmptyArray(forMe)){
    ...
    this.dataInitiatedForMe = forMe;
    this.dataInitiatedForMe = this.dataInitiatedForMe.slice();
}
