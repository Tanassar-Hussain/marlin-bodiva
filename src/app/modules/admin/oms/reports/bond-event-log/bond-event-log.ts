...
export class BondEventLog implements OnInit, AfterViewInit {
    ...
    updateMarketData(markets) {
        ...
        for (let i = 0; i < markets.length; i++) {
            if (markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_BOND || markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_QUOTE_STRING || markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_AUCTION) {
        ...
        if (this.markets.length > 0) {
            ...
            this.markets.forEach((element) => {
                if (element.marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_BOND) {
                    setTimeout(() => {
