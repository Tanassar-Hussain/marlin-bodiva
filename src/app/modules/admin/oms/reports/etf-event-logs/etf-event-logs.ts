...
@Component({
    selector: 'etf-event-logs',
    templateUrl: './etf-event-logs.html',
    encapsulation: ViewEncapsulation.None
})
export class ETFsEventLog implements OnInit, AfterViewInit {
        ...
        this.listingSvc.getUserList(AppConstants.participantId).subscribe(
            (users : any) => {
                ...
                if (AppUtility.isEmptyArray(users)) {
                    ...
                    users = users.filter(user => user.userName != AppConstants.loginName);
    ...
    updateData(eventLog): void {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            if (((AppUtility.isEmpty(this.exchangeCode) || this.exchangeId === 0) || this.exchangeCode === orders[i].exchange) &&
                ...
                if (orders[i].order_state !== 'partial_filled' && orders[i].order_state !== 'filled') {
                    ...
                    orders[i].state_time = new Date(orders[i].state_time + 'Z');
                ...
                if (orders[i].order_state !== 'partial_filled' && orders[i].order_state !== 'filled') {
                    ...
                    orders[i].settlementValue = (p) * orders[i].volume;
                    this.data_temp.push(orders[i]);
                }
            }
        }
        ...
        for (let j = 0; j < trades.length; j++) {
            ...
            if (((AppUtility.isEmpty(this.exchangeCode) || this.exchangeId === 0) || this.exchangeCode === trades[j].exchange) &&
                ...
                if (trades[j].remaining_volume > 0) {
                    trades[j].order_state = 'Partial Filled';
                }
                else {
                ...
                trades[j].state_time = new Date(trades[j].execution_time + 'Z');
                trades[j].execution_time = new Date(trades[j].execution_time + 'Z');
        ...
        this.data = this.data_temp.sort((n1, n2) => new Date(n2.state_time).getTime() - new Date(n1.state_time).getTime());
    ...
    fillTradeInfo(orderData, trades) {
        ...
        for (let i = 0; i < trades.length; i++) {
            if (orderData.order_no === trades[i].order_no && (orderData.order_state === 'filled' || orderData.order_state === 'partial_filled')) {
                ...
                orderData.execution_time = new Date(trades[i].execution_time + 'Z');
    ...
    updateMarketData(markets) {
        ...
        for (let i = 0; i < markets.length; i++) {
            if (markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_ETF_ || markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_QUOTE_STRING || markets[i].marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_AUCTION ) {
        ...
        if (this.markets.length > 0) {
            ...
            this.markets.forEach((element)=>{
                if(element.marketType.description.toUpperCase() === AppConstants.MARKET_TYPE_ETF_){
                    setTimeout(() => {
