...
@Component({
    selector: 'etf-working-orders',
    templateUrl: './etf-working-orders.html',
    encapsulation: ViewEncapsulation.None,
})
export class ETFsWorkingOrders implements OnInit, AfterViewInit {
            ...
            if (this.userType === 'PARTICIPANT' || this.userType === 'PARTICIPANT ADMIN') {
                if (!this.allTraders) {
                    ...
                    if (this.cmbTraders.checkedItems.length > 0) {
                        ...
                        usersOmsReports.users[0] = this.username
                    }
    ...
    updateorderConfirmation(data) {
        if (AppUtility.isValidVariable(data)) {
            if (data.state === 'submitted' || data.state === 'changed' || data.state === 'cancelled' || data.state === 'trade') {
