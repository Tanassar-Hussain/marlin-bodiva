...
export class MarketWatchEB implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild('flexGridMWETF',{ static: true }) flexGridMWETF: wjcGrid.FlexGrid;
  ...
  itemFormatterETF = (panel: wjcGrid.GridPanel, r: number, c: number, cell: HTMLElement) => {
    let value;
    ...
    if (panel.cellType === wjcGrid.CellType.Cell) {
      ...
      // buy volume
      // if ( c ==3 && Number(JSON.stringify(panel.rows[r].dataItem["sell_volume"]))>=0) {
      if (c === 3 && this.dataMWETF[r].buy_volume !== this.dataMWETF[r].buy_volume_old) {
        this.blinkCell(cell, this.dataMWETF[r].buy_volume, this.dataMWETF[r].buy_volume_old);
        this.dataMWETF[r].buy_volume_old = this.dataMWETF[r].buy_volume;
      }

      // buy price
      if (c === 4 && Number(this.dataMWETF[r].buy_price) !== Number(this.dataMWETF[r].buy_price_old)) {
        this.blinkCell(cell, this.dataMWETF[r].buy_price, this.dataMWETF[r].buy_price_old);
        this.dataMWETF[r].buy_price_old = this.dataMWETF[r].buy_price;
      }

      // sell price
      if (c === 6 && Number(this.dataMWETF[r].sell_price) !== Number(this.dataMWETF[r].sell_price_old)) {
        this.blinkCell(cell, this.dataMWETF[r].sell_price, this.dataMWETF[r].sell_price_old);
        this.dataMWETF[r].sell_price_old = this.dataMWETF[r].sell_price;
      }

      // sell volume
      if (c === 8 && this.dataMWETF[r].sell_volume !== this.dataMWETF[r].sell_volume_old) {
        this.blinkCell(cell, this.dataMWETF[r].sell_volume, this.dataMWETF[r].sell_volume_old);
        this.dataMWETF[r].sell_volume_old = this.dataMWETF[r].sell_volume;
      }
    }
  }
  ...
  couponRateBond: any = null;
  nextCouponDateBond: any = null;
  maturityDateBond: any = null;
  exchangeId: any;
  marketId: any;
  securityId: any;
  symbolMarketDisplayName: any;
  equitiesWatchItems: any[] = [];
  bondsWatchItems: any[] = [];
  etfWatchItems: any[] = [];
  ...
  ngAfterViewInit() {
    ...
    jQuery('.editOrder_btn').click(function () {
      let assetId = self.dataService.getAssetTypeId(self.selectedOrder.exchange,
        self.selectedOrder.market,
        self.selectedOrder.symbol);
      if (assetId == AppConstants.ASSET_CLASS_ID_EQUITIES) {
    ...
    if (this.flexGridMWETF) {
      this.updateDataMapSettings('ETF');
    }
...
public getWatchItems = () => {
  
      let watchItems : any = null;
      const  a = localForage.getItem(AppConstants.username).then((element : any)=>{
           watchItems = element?.watchItems;
           this.loadMarketWatchEquitySymbolsFromStorage(watchItems?.equities);
           this.loadMarketWatchBondSymbolsFromStorage(watchItems?.bonds);
           this.loadMarketWatchETFSymbolsFromStorage(watchItems?.ETF);
  ...
  loadMarketWatchEquitySymbolsFromStorage(symbolList) {
    ...
    if (AppUtility.isValidVariable(symbolList)) {
      for (let i = 0; i < symbolList.length; i++) {
        ...
        symbol = (typeof strArr[0] === 'undefined') ? '' : strArr[0];
        market = (typeof strArr[1] === 'undefined') ? '' : strArr[1];
        exchange = (typeof strArr[2] === 'undefined') ? '' : strArr[2];

        let assetId = this.dataService.getAssetTypeId(exchange, market, symbol);
        if (assetId == AppConstants.ASSET_CLASS_ID_EQUITIES) {
  ...
  loadMarketWatchBondSymbolsFromStorage(symbolList) {
    ...
    if (AppUtility.isValidVariable(symbolList)) {
      for (let i = 0; i < symbolList.length; i++) {
        ...
        let assetId = this.dataService.getAssetTypeId(exchange, market, symbol);
        if (assetId === AppConstants.ASSET_CLASS_ID_BONDS) {
          obj.exchangeCode = exchange;
          obj.marketCode = market;
          obj.symbol = symbol;
  ...
  loadMarketWatchETFSymbolsFromStorage(symbolList) {
    ...
    //    strArr = symbolList[i].split(AppConstants.LABEL_SEPARATOR);
        ...
        let assetId = this.dataService.getAssetTypeId(exchange, market, symbol);
        if (assetId === AppConstants.ASSET_CLASS_ID_ETFS) {
          obj.exchangeCode = exchange;
          obj.marketCode = market;
          obj.symbol = symbol;
  ...
  initMarketWatch(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.dataMWETF[i] = new MarketWatch();
    }
  }
      ...
      this.orderSvc.getBestMarketAndSymbolStats(exchangeCode, marketCode, securityCode)
        .subscribe(data => {
          if (AppUtility.isValidVariable(data)) {
            let assetId = this.dataService.getAssetTypeId(exchangeCode, marketCode, securityCode);
            if (assetId === AppConstants.ASSET_CLASS_ID_BONDS) {
              
              if(AppUtility.isValidVariable(this.symbolMarketExchange)){
                this.symbolExangeMarketList.forEach(element => {
                   if(this.symbolMarketExchange === element.displayName_){
             ...
             this.listingSvc.getSymbolMarket(exchangeId, marketId, securityId).subscribe((res:any)=>{
              this.couponRateBond = res.fisDetail.couponRate;
              this.nextCouponDateBond = res.nextCouponDate;
              this.maturityDateBond = res.fisDetail.maturityDate; 
              this.symbolMarketDisplayName = res.displayName_;
              
                this.updateBestMarketAndSymbolStatsBond(data , res.fisDetail.couponRate , res.nextCouponDate, res.fisDetail.maturityDate, res.displayName_);
             
              
            });
            }
            else if(assetId === AppConstants.ASSET_CLASS_ID_EQUITIES){
              this.updateBestMarketAndSymbolStatsEquity(data);
            }
            else if(assetId === AppConstants.ASSET_CLASS_ID_ETFS){
             ...
             this.listingSvc.getSymbolMarket(exchangeId, marketId, securityId).subscribe((res:any)=>{
              ...
              this.symbolMarketDisplayName = res.displayName_;
                ...
                this.updateBestMarketAndSymbolStatsETFs(data , res.fisDetail.couponRate , res.nextCouponDate, res.fisDetail.maturityDate, res.displayName_);
  ...
  updateBestMarketAndSymbolStatsEquity(data) {
    ...
    if (AppUtility.isValidVariable(data) &&
      !AppUtility.isEmpty(data) && AppUtility.isValidVariable(this.flexGridMWEquity.collectionView)) {
  ...
  updateBestMarketAndSymbolStatsBond(data , cR , nCR, mD , dN) {
    
    let item: MarketWatch;
    if (AppUtility.isValidVariable(data) && !AppUtility.isEmpty(data) && AppUtility.isValidVariable(this.flexGridMWBond.collectionView)) {
      for (let i = 0; i < this.flexGridMWBond.collectionView.items.length; i++) {
        ...
        if (AppUtility.isValidVariable(item.exchangeCode) && item.exchangeCode.length > 0
          ...
          let a = item.symbol+"("+item.marketCode+"/"+item.exchangeCode+")";

          if(dN === a){
            
            item.updateCouponMaturityfields(cR, nCR, mD);
          }
  ...
  updateBestMarketAndSymbolStatsETFs(data , cR , nCR, mD , dN) {
    
    let item: MarketWatch;
    if (AppUtility.isValidVariable(data) && !AppUtility.isEmpty(data) && AppUtility.isValidVariable(this.flexGridMWETF.collectionView)) {
      for (let i = 0; i < this.flexGridMWETF.collectionView.items.length; i++) {
        item = <MarketWatch>this.flexGridMWETF.collectionView.items[i];
  ...
  itemsSourceChangedHandler(marketType) {
    ...
    else if(marketType == 'Bond')
      flex = this.flexGridMWBond;
     else if(marketType == 'ETF') 
     flex = this.flexGridMWETF;
  ...
  selectionChangedMWETF(e) {
    if (AppUtility.isValidVariable(this.flexGridMWETF.collectionView.currentItem) &&
      AppUtility.isValidVariable(this.flexGridMWETF.collectionView.currentItem.exchangeCode) &&
      AppUtility.isValidVariable(this.flexGridMWETF.collectionView.currentItem.marketCode) &&
      AppUtility.isValidVariable(this.flexGridMWETF.collectionView.currentItem.symbol)) {
      this.exchange = this.flexGridMWETF.collectionView.currentItem.exchangeCode;
      this.market = this.flexGridMWETF.collectionView.currentItem.marketCode;
      this.symbol = this.flexGridMWETF.collectionView.currentItem.symbol.toUpperCase();
  ...
  cellEditEndedHandlerETF(e) {
    // first column is exchange
    let validSymbol: boolean = false;

    if (e.col == 0) {
      this.getExchangeData(this.flexGridMWETF.collectionView.currentItem.exchangeCode);
    }

    if (e.col === 2) {
      this.flexGridMWETF.collectionView.currentItem.symbol = this.flexGridMWETF.collectionView.currentItem.symbol.toUpperCase();
      this.exchange = this.flexGridMWETF.collectionView.currentItem.exchangeCode;
      this.market = this.flexGridMWETF.collectionView.currentItem.marketCode;
      this.symbol = this.flexGridMWETF.collectionView.currentItem.symbol.toUpperCase();

      let item = <MarketWatch>this.flexGridMWETF.collectionView.currentItem;
      ...
      if (this.dataService.isValidSymbol(item.exchangeCode, item.marketCode, item.symbol)) {
        ...
        if (this.flexGridMWETF.selection.row > this.marketWatchCurrentRowIndex) {
          this.marketWatchCurrentRowIndex = this.flexGridMWETF.selection.row;
        }
      }
      else {
        item.symbol = '';
      ...
      if (validSymbol) {
        this.flexGridMWETF.collectionView.currentItem = item;
        this.dataService.setExchMktSymbol(this.exchange, this.market, this.symbol);
  ...
  onRemoveSymbolToWatch() {
   ...
   if(this.isETF){
    if (AppUtility.isValidVariable(this.flexGridMWETF.selection.row)) {
      let rowIndex = this.flexGridMWETF.selection.row;
      let j: number;
      let item = <MarketWatch>this.flexGridMWETF.collectionView.currentItem;
      let lastItem: MarketWatch;
      // working code
      for (let i = rowIndex; i < this.dataMWETF.length - 1; i++) {
        j = i;
        if (i >= rowIndex) {
          j = i + 1;
          this.flexGridMWETF.rows[i].dataItem = this.flexGridMWETF.rows[j].dataItem;
        }
  ...
  public getMarketWatchEmptyRowIndex(marketType): number {
    ...
    if (marketType == 'Bond') {
      ...
      for (let i = 0; i < this.flexGridMWETF.rows.length; i++) {
        item = <MarketWatch>this.flexGridMWETF.rows[i].dataItem;
        if (!AppUtility.isValidVariable(item.symbol) || item.symbol.length === 0) {
          emptyIndex = i;
          return emptyIndex;
        }
  ...
  private _buildMarketDataMap(items, marketType): wjcGrid.DataMap {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      else if(items[i].marketType.description === AppConstants.MARKET_TYPE_EQUITIES)
        mapEquity.push({ key: items[i].marketCode, value: items[i].marketCode });
        else if(items[i].marketType.description === AppConstants.MARKET_TYPE_ETF_)
        mapETF.push({ key: items[i].marketCode, value: items[i].marketCode });
    }
    if (marketType == 'Bond')
      ...
      return new wjcGrid.DataMap(mapETF, 'key', 'value');
  }
  ...
  saveSymbolsToStorage(marketType) {
    ...
    if (marketType == 'Bond') {
      ...
      for (let i = 0; i < this.flexGridMWETF.rows.length; i++) {
        let item = <MarketWatch>this.flexGridMWETF.rows[i].dataItem;
  ...
  private populateSymbolExangeMarketList(_participantId: number) {
    ...
    if(AppUtility.isValidVariable(AppConstants.participantId)){
      this.listingSvc.getParticipantSecurityExchanges(_participantId).subscribe(restData => {
        this.splash.hide();
  ...
  addRemoveFromStorage() {
    
    var exchangemarketsymbolsEquity : any[] = [];
    var exchangemarketsymbolsBonds : any[] = [];
    var exchangemarketsymbolsETF : any[] = [];

    if(AppUtility.isValidVariable(this.flexGridMWEquity)){
      const rows = this.flexGridMWEquity.rows;
      rows.forEach((element)=>{
         if(!AppUtility.isNullOrEmpty(element.dataItem.exchangeCode) && !AppUtility.isNullOrEmpty(element.dataItem.marketCode) && !AppUtility.isNullOrEmpty(element.dataItem.symbol)){
          exchangemarketsymbolsEquity.push(AppUtility.symbolMarketExchangeComb(element.dataItem.symbol , element.dataItem.marketCode, element.dataItem.exchangeCode));
         }
      })
    
    }
  
   if(AppUtility.isValidVariable(this.flexGridMWBond)){
    const rowsBond = this.flexGridMWBond.rows
    rowsBond.forEach((element)=>{
       if(!AppUtility.isNullOrEmpty(element.dataItem.exchangeCode) && !AppUtility.isNullOrEmpty(element.dataItem.marketCode) && !AppUtility.isNullOrEmpty(element.dataItem.symbol)){
        exchangemarketsymbolsBonds.push(AppUtility.symbolMarketExchangeComb(element.dataItem.symbol , element.dataItem.marketCode, element.dataItem.exchangeCode));
       }
    })
   }
   
    
   if(AppUtility.isValidVariable(this.flexGridMWETF)){
    const rowsETF = this.flexGridMWETF.rows
    rowsETF.forEach((element)=>{
       if(!AppUtility.isNullOrEmpty(element.dataItem.exchangeCode) && !AppUtility.isNullOrEmpty(element.dataItem.marketCode) && !AppUtility.isNullOrEmpty(element.dataItem.symbol)){
        exchangemarketsymbolsETF.push(AppUtility.symbolMarketExchangeComb(element.dataItem.symbol , element.dataItem.marketCode, element.dataItem.exchangeCode));
       }
    })
   }
    ...
    localForage.setItem(AppConstants.username , {watchItems : {
      equities : exchangemarketsymbolsEquity,
      bonds : exchangemarketsymbolsBonds,
      ETF : exchangemarketsymbolsETF
    }});
  ...
  private addSymbolToWatch() {
    ...
    this.couponRateBond = null;
    this.nextCouponDateBond = null;
    this.maturityDateBond = null;
     ...
     let strArr = AppUtility.isSplitSymbolMarketExchange(this.symbolCombo.selectedValue);
      ...
      if (assetId === AppConstants.ASSET_CLASS_ID_BONDS) {
       ...
       //  this.getSymbolMarketDetail(exchangeId , marketId , securityId);
        ...
        let rowIndex = this.getMarketWatchEmptyRowIndex('ETF');
        let item = new MarketWatch();
        this.marketWatchCurrentRowIndex = rowIndex;
        ...
        item.marketCode = this.market;
        item.symbol = this.symbol;
        this.dataMWETF[rowIndex] = item;
        this.flexGridMWETF.rows[rowIndex].dataItem = item;
        this.flexGridMWETF.invalidate();
 ...
 public getSymbolMarketDetail=(exchangeId , marketId , securityId)=>{
  ...
  public onAddSymbolToWatch() {
    ...
    if (this.isBond === true) {
      if (AppUtility.isValidVariable(this.symbolExangeMarketWiseList) && !AppUtility.isEmptyArray(this.symbolExangeMarketWiseList)) {
        for (let i = 0; i < this.symbolExangeMarketWiseList.length; i++) {
          if (this.symbolExangeMarketWiseList[i].assetId === AppConstants.ASSET_CLASS_ID_BONDS) {
    ...
    else if(this.isEquity === true) {
      if (AppUtility.isValidVariable(this.symbolExangeMarketWiseList) && !AppUtility.isEmptyArray(this.symbolExangeMarketWiseList)) {
        for (let i = 0; i < this.symbolExangeMarketWiseList.length; i++) {
          if (this.symbolExangeMarketWiseList[i].assetId === AppConstants.ASSET_CLASS_ID_EQUITIES) {
            symbols[j] = this.symbolExangeMarketWiseList[i];
            symbols[j].id = this.symbolExangeMarketWiseList[i].exchangeMarketSecurityId;
            symbols[j].value = this.symbolExangeMarketWiseList[i].displayName_;
            j++;
    ...
    else if(this.isETF === true) {
      if (AppUtility.isValidVariable(this.symbolExangeMarketWiseList) && !AppUtility.isEmptyArray(this.symbolExangeMarketWiseList)) {
        for (let i = 0; i < this.symbolExangeMarketWiseList.length; i++) {
          if (this.symbolExangeMarketWiseList[i].assetId === AppConstants.ASSET_CLASS_ID_ETFS) {
            symbols[j] = this.symbolExangeMarketWiseList[i];
            symbols[j].id = this.symbolExangeMarketWiseList[i].exchangeMarketSecurityId;
            symbols[j].value = this.symbolExangeMarketWiseList[i].displayName_;
            j++;
