...
@Component({
    selector: 'app-rejected-orders-list',
    templateUrl: './rejected-orders-list.component.html',
    styleUrls: ['./rejected-orders-list.component.scss']
})
export class RejectedOrdersListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    @ViewChild('dialogCmp') dialogCmp: DialogCmpReports;
    ...
    onFilter(showEmptyMsg: boolean){
        this._fetchRejectedOrders(showEmptyMsg);
    }
    ...
    private _fetchRejectedOrders(showEmptyMessage = false) {
        ...
        this._listingService.getRejectedOrder(filters).pipe(takeUntil(this._destroy$)).subscribe({
            next: (orders: any) => {
