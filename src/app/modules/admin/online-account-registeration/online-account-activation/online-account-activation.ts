...
export class OnlineAccountActivation implements OnInit {
 ...
 public getTranslation=()=>{
  this.translate.get(['Translation.Select']).subscribe((res:any)=>{
...
public updateClientStatus = (clientIdForStatus : Number)=> {
  ...
  public onSaveAction(model: any, isValid: boolean) {
    ...
    if (!AppUtility.isValidVariable(this.selectedItem.user.userId)) {
      if(this.lang === 'pt'){
        this.dialogCmp.statusMsg = 'ID de usuário inválido.';
      }
      else{
   ...
   //==================Account Configuration Section 
    ...
    // If user selected active and changed the status 
 ...
 // c.accountTypeNew.accTypeId = data.accountType;  //Comment By Faizan --- 22-03-2023
    c.clientId = data.clientId;
  ...
  private addFromValidations() {
    this.myForm = this._fb.group({
      ...
      phone1: ['', Validators.compose([Validators.required, this.removeSpaces])],
