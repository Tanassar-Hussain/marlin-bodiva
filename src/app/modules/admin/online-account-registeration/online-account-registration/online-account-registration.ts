...
export class OnlineAccountRegistration implements OnInit {
  ...
  @ViewChild('dialogCmp') dialogCmp: DialogCmpRegistration;
  ...
  disabledSaveDraft: boolean = false;
  ...
  public showApproveConfirmation = (action) => {
    ...
    if (this.showCorporate) {
      this.investorNameForAR = this.selectedItem.contactDetail.companyName;
  ...
  public onGetDraft = () => {
    ...
    this.isDisabledUpdateReject = false;
  ...
  public onSaveDepository = () => {
    ...
    // 
    //  this.selectedDepositoryItem.REF_No = String(this.selectedItem.clientId)+ "-"+ milliseconds;
    ...
    this.listingService.invCDCRegister(this.selectedDepositoryItem).subscribe((data: any) => {
      ...
      if (error.Message) {
        ...
        if (JSON.stringify(wordsToPick) === JSON.stringify(uniqueArray)) {
          this.changeStatusSendDepositoryAlreadyExists();
          this.dialogCmp.statusMsg = this.errorMessage;
    ...
    if (!AppUtility.isEmptyArray(cdcRecordForFilter)) {
      ...
      for (let i = 0; i < cdcRecordForFilter.length; i++) {
        ...
        if (cdcRecordForFilter[i].statusCode === AppConstants.INV_STATUS_PENDING) {
          ...
          this.invCDCStatus[a].participant_Code = cdcRecordForFilter[i].participantCode;
  ...
  public hideModalTermsConditionsAgree() {
    (jQuery("#term_conditions") as any).modal("hide");   //hiding the modal on agree terms and conditions
    this.pdf = false;
    this.selectedItem.agreeTermsAndConditions = true;
    this.tempParticipantId = this.selectedItem.participant?.participantId;
  }


  public hideModalTermsConditionsDisagree() {
    (jQuery("#term_conditions") as any).modal("hide");   //hiding the modal on disagree terms and conditions
    this.pdf = false;
    this.selectedItem.agreeTermsAndConditions = false;
    this.tempParticipantId = null;
  ...
  private showModalTermConditions = (participantId) => {
    if (AppUtility.isValidVariable(participantId)) {
      let langId;
      if (this.lang == 'en') {
        langId = AppConstants.LANGUAGE_ID_ENGLISH
      } else if (this.lang == 'pt') {
        langId = AppConstants.LANGUAGE_ID_PORTUGUESE
      }
      this.splash.show();
  ...
  private fillClientFromJson(c: Client, data: any) {
    ...
    if (!AppUtility.isNullOrEmpty(data.clientCode)) {
      c.clientCode = data.clientCode;
    }
    else {
      c.clientCode = AppConstants.PARTICPANT_BANK_CODE;
    }
  ...
  private populateItemGrid() {
    ...
    if (AppUtility.isEmpty(this.searchInvestorTaxNumber) && AppUtility.isEmpty(this.searchInvestorName)) {
      this.listingService.getInvestorListByParticipantMob(AppConstants.participantId)
        .subscribe(
          restData => {
            this.splash.hide();
      ...
      this.listingService.getInvestorListBySearchMob(AppConstants.participantId, AppConstants.ALL_VAL, encodeURIComponent(this.searchInvestorTaxNumber))
        .subscribe(
          restData => {
            this.splash.hide();
      ...
      this.listingService.getInvestorListBySearchMob(AppConstants.participantId, encodeURIComponent(this.searchInvestorName), AppConstants.ALL_VAL)
        .subscribe(
          restData => {
            this.splash.hide();
      ...
      this.listingService.getInvestorListBySearchMob(AppConstants.participantId, encodeURIComponent(this.searchInvestorName), encodeURIComponent(this.searchInvestorTaxNumber),)
        .subscribe(
          restData => {
  ...
  private populateParticipants() {
    ...
    this.listingService.getParticipantListByExchagne(AppConstants.exchangeId).subscribe(restData => {
      ...
      if (this.showParticipantBasedGrid) {
        ...
        if (AppUtility.isValidVariable(this.selectedParticipantId) && this.selectedItem.statusCode === 'D') {
        ...
        else if (AppUtility.isValidVariable(this.selectedParticipantId) && this.selectedItem.statusCode !== 'D') {
          setTimeout(() => {
            this.selectedItem.participant.participantId = this.selectedParticipantId;
          }, 150);
        }
        else if (AppUtility.isValidVariable(AppConstants.BROKER_CODE_SIGN_IN) && AppConstants.BROKER_CODE_SIGN_IN !== "") {

          for (let i = 0; i < this.participantsList.length; i++) {
            if (this.participantsList[i].participantCode == AppConstants.BROKER_CODE_SIGN_IN) {
              tempArr.push(this.participantsList[i]);
            }
          }
          this.participantsList = tempArr;
          this.getSettlementMembersCDS(AppConstants.BROKER_CODE_SIGN_IN);
          setTimeout(() => {
  ...
  private validateBasicInfo(): boolean {
    if (this.showIndividual && this.showTrustee) {
      ...
      if (AppUtility.isEmpty(this.selectedItem.contactDetail.firstName) ||
        ...
        AppUtility.isEmpty(this.selectedItem.accountTypeNew.accTypeId) ||
        (AppUtility.isEmpty(this.selectedItem.clientCode) && this.showParticipantBasedGrid) ||
        (AppUtility.isEmpty(this.selectedItem.settlementMemberCode) && this.showParticipantBasedGrid) ||
        (AppUtility.isEmpty(this.selectedItem.eventSettlementMemberCode) && this.showParticipantBasedGrid)) {
        return false;
      }
      else {
  ...
  private validateSystemAccess(): boolean {
    ...
    if (this.selectedItem.onlineClient && !this.isEditing) {
      if (AppUtility.isEmpty(this.selectedItem.user.userName) ||
        ...
        if (this.currentTab_ != "SA") {
          if (this.lang === 'pt') {
            this.dialogCmp.statusMsg = 'Por favor, preencha todos os campos obrigatórios da aba Usuário.';
          }
          else {
        ...
        if (this.currentTab_ != "SA") {
          if (this.lang === 'pt') {
            this.dialogCmp.statusMsg = 'Incompatibilidade de senha e confirmação de senha.';
          }
          else {
