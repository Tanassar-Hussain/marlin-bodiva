...
export class TradingPortalService {
    ...
    getKlineGraphDataDynamic(exchange: string, symbolCode: string, dataLength: number) {
        return this.restService.get(false, `security/symbol/stats-date-range/exch/${exchange}/${symbolCode}/limit/${dataLength}`).pipe(map((res => {
            return res.map(u => this.tradingGraphData.adapt(u));
        })))
    }


    getPeriodicData(exchangeCode: string, marketCode: string, symbolCode: string) {
        return this.restService.get(false, `allSymbolStats/exchange/${exchangeCode}/market/${marketCode}/symbol/${symbolCode}`)
    }
}
