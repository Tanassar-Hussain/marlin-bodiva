...
export class ListingService {
  constructor(private appUtility: AppUtility, private appCons: AppConstants, public translate: TranslateService, private restService : RestService) { }
  urlString: string;
  ...
  public getParticipantSecurityExchangesNew(brokerId: number): Observable<Object[]> {
    this.urlString = 'participants/' + brokerId + '/exchange-market-securities/';
    return this.appUtility.wsGetRequest(this.urlString);
  }

  public getexchangeCodeSecurityExchanges(): Observable<Object[]> {
    this.urlString = 'exchanges/' + AppConstants.exchangeCode + '/exchange-market-securities-new/';
    return this.appUtility.wsGetRequest(this.urlString);
  }
  ...
  public securityMarketTrades(securityCode: string, date): Observable<Object[]> {
    this.urlString = `security/vtstats/security/${securityCode}/tradedate/${date}`;
    return this.appUtility.wsGetRequest(this.urlString);
  }
  ...
  public getClientListRpt(participantId: Number, fromDate: Date, toDate: Date, lang: String, reportFormat : String): Observable<Object[]> {
    this.urlString = null;
    let from_Date = new DatePipe('en-US').transform(fromDate, 'yyyy-MM-dd');
    let to_Date = new DatePipe('en-US').transform(toDate, 'yyyy-MM-dd');
   this.urlString = 'reports/clients/participant/' + participantId + '/from-date/' + from_Date + '/to-date/' + to_Date + '/lang/' + lang + '/'+ 'reportFormat/'+reportFormat+'/'
    return this.appUtility.wsGetRequest(this.urlString);
  }
