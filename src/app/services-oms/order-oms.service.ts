...
export class OrderService {
  ...
  public cancelQuoteOrder(quote): any {
    let url = AppConstants.omsBaseUrl + 'transactions/quote';
    let Httpheaders = new HttpHeaders();
    Httpheaders = Httpheaders.append('Authorization', 'Bearer ' + OrderService.token);
    return this.http.delete(url, { headers: Httpheaders, body: quote });
  ...
  public getAvailableHolding(exchangeCode: string, participantCode: string , user : string ,clients: string, security: string): any {
    let url = AppConstants.omsBaseUrl + 'risk/clients/exchanges/' + exchangeCode + '/brokers/' + participantCode + '/users/' + user + '/clients/' + clients + '/security/' + security + '/holding/';
  ...
  public getSingleEventLog(orderNo): any {
    let users = {};
    let url: string = AppConstants.omsBaseUrl + 'transactions/eventlog/' + orderNo;
